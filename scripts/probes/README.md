...
| `body-tokens.smoke.mjs` | **Drafts only: this probe never sends.** The `{{signature}}` round trip against a real identity - the configured sign-off is what lands, it lands once at write, and a body read back carries no token to expand again; the escape ships bare braces through the derived text part too; the four `draft_email` token refusals (near-miss, wrong mode, repeated, one supplied part); and `edit_draft`'s `bodyHash` gate - refused with none and with a stale one, the next edit's hash handed back, `{{signature}}` stored as written unless `expandSignature` is passed. Recipients are addressed into `invalid.example`, which publishes no MX, so nothing is deliverable even by accident. Ran in full against a live account on 2026-09-02, every check passing |
...
| `calendar-uid-query.probe.mjs` | The premise behind #137's targeted lookup: whether Fastmail's CalDAV server accepts a `calendar-query` filtered on `UID` with a `text-match match-type="equals"`, and whether it HONORS `equals` rather than falling back to CalDAV's default `contains`. The fix proposed for #137 would replace `findCalendarObjectByUID`'s full-account scan with one such query per calendar and would have no fallback, so both halves have to hold. Queries an event that already exists for its exact UID (which must come back with non-empty `data` and `etag`), then for a strict substring of that UID and for a UID that cannot exist (both of which must return nothing). The substring query is the load-bearing one: a server matching more loosely than asked would return resources nobody asked for, manufacturing duplicate "copies" in an ambiguity count two destructive tools rest on. Also measures — without gating on it — whether the default collation's ASCII case-insensitivity reaches `equals` matching, by querying a case-variant UID: on this account it does — the case-variant query matched the same resource. Raw CalDAV via tsdav, not the built server, so it measures the platform rather than our parsing. Read-only; creates, updates and deletes nothing, and needs no fixture. Output is resource COUNTS and PASS/FAIL only — no URL, UID, title or other account value is printed, including inside a server error string — so a run can be quoted verbatim into a public issue or commit |
