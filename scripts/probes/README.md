...
- One probe is a deliberate exception to the first rule.
  ...
  temporary collection and exits with them in place; its `cleanup` phase deletes that
  whole collection, taking every event inside it in one request. Nothing else sweeps
  them, so a `create` that is never followed by a `cleanup` leaves a stray calendar
  behind. Run the two phases as a pair.
