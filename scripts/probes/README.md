...
| `calendar-window.probe.mjs` | The other half of #64, end to end: whether the shipped `list_calendar_events` answers a window question with dates that are actually in that window. Asserts every returned `start` falls inside the requested range, that a recurring entry arrives marked as an expanded occurrence rather than as the series master it used to be reported as, and that the response carries a total so a trimmed list is visible (#100). Spawns the BUILT server through the MCP harness, so unlike `calendar-expand.probe.mjs` it measures our parsing and filtering rather than the platform. Read-only; creates nothing |
| `signature.smoke.mjs` | Identity signature on compose (#33): the `fm-mcp-signature` marker survives create/store/fetch on the real server, a signed reply places it above the quote in both body parts, an `htmlBody`-alone `edit_draft` re-appends it (exactly one block, still above the quote) and announces it, `appendSignature:false` removes it, and an unflagged compose stays unsigned. Drafts only, no send |
