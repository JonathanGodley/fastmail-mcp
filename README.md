...
- **List parameters take a bare string too** - a single value, a comma-separated string or a JSON-encoded array all coerce to an array. That covers `to`/`cc`/`bcc`/`replyTo`, `emailIds`/`mailboxes`, `clearFields`, `removeAttachments`, `fields`, and `inReplyTo`/`references` on `draft_email`'s `mode:'new'`.
...
>
> **Recipient format:** every recipient field (`to`/`cc`/`bcc`/`replyTo` on `draft_email` and `edit_draft`) accepts each entry as either a bare address (`a@x.example`) or the RFC 5322 `"Name <email>"` form (`Alice <a@x.example>`), which is parsed into a display name + address. The SMTP envelope always uses the bare address.
>
