...
2. Set environment variables:
   ...
   # An unrecognised name is not an error - it falls back to the host's timezone.
...
>
> **Draft sender name:** drafts created or edited via `create_draft`/`edit_draft` carry a display name alongside the From address, so the From shows your name rather than a bare address. On an edit the name is resolved in the same order the address is: the verified identity that owns the address it writes, else the name the stored draft already carried against that same address, else none. The last step matters — a stored `from` matching no verified identity (an address removed from the account, or a draft made elsewhere) keeps its own address and gets **no** display name, rather than borrowing the default identity's name and putting it in front of a foreign address. See [`docs/email-bodies.md`](docs/email-bodies.md).
...
- **edit_draft**: Edit an existing draft email. Since JMAP emails are immutable, this creates a replacement draft and moves the old one to Trash (so the returned email ID is new). The edit preserves the draft's threading headers (In-Reply-To/References), attachments, and other keywords. The replaced draft is **never destroyed** — it stays in Trash until Trash is emptied or auto-purged (Fastmail's Trash retention is a per-account setting, and can be set to never), so an edit made from a stale copy (the draft changed in the web UI or another client in between) can be undone. The result also reports what the replaced draft held (subject, recipients, body sizes); compare that against what you meant to replace to spot an unintended overwrite ([#65](https://github.com/JonathanGodley/fastmail-mcp/issues/65)). On the rare failure where the replacement is created but the old copy can't be moved to Trash, you are left with a duplicate draft rather than none, and the result says so. Only fields you provide are changed; omit a field to leave it unchanged. A signature already on the draft is preserved across a body edit — the identity's current signature is re-appended and the result says so — unless you pass `appendSignature:false` to drop it (see [Signing a message](#signing-a-message)).
  ...
  - Parameters: `emailId` (required), `to` (optional array), `cc` (optional array), `bcc` (optional array), `from` (optional), `subject` (optional), `textBody` (optional), `htmlBody` (optional), `replyTo` (optional array), `appendSignature` (optional boolean, TRI-STATE — omit to preserve a signature the draft carries, `true` to add one, `false` to drop it; see [Signing a message](#signing-a-message)), `clearFields` (optional array), `attachments` (optional array — **appends**), `removeAttachments` (optional array of blobId/name), `originalEmailId` (optional — for reply-quote preservation, below), `noQuote` (optional boolean)
  ...
  - **Reply-quote / forwarded-block preservation (`originalEmailId` / `noQuote`).** A reply draft keeps the quoted original inside its body — in the `htmlBody` for an HTML draft, or as a `> `-prefixed block in the `textBody` for a text-only one; a forward draft keeps the forwarded-message block the same way. Editing the body of a draft that **still carries** its quoted/forwarded original would silently discard it, so the edit is **rejected** unless you say what to do: pass `originalEmailId` (the id of the message the draft replies to or forwards — *not* the draft's own `emailId`) to regenerate the body and **keep** it, or `noQuote: true` to deliberately **drop** it. The check is made on the draft's *existing* body (the shape this server generated), not on your new text, so it can't be bypassed by including quote-shaped content. Two edits keep the quote/block automatically and need neither flag: a metadata-only edit (subject/recipients/attachments), and a plain-text conversion (`clearFields:['htmlBody']`, which leaves the text-side quote/block in place). Supplying `htmlBody` to a text-only reply draft converts it to HTML. The regenerated body re-quotes the named original from scratch (it never reassembles the stored quote). Each successive keep-edit must pass `originalEmailId` again.
    - **If your new body already contains the quote, use `noQuote` instead.** The rebuild **appends** the fresh quote to the body you supply; it does not notice one already in there. So the natural read-edit-write loop — read the draft, change the words, hand the whole text back — must pass `noQuote: true` (your body is already carrying the original), not `originalEmailId`, or the draft ends up with the quoted original **twice**. Pass `originalEmailId` when your new body is just your own text. Detection is tracked as [#145](https://github.com/JonathanGodley/fastmail-mcp/issues/145).
...
- **remove_labels**: Remove specific labels (mailboxes) from an email. Removing the message's last label archives it instead of leaving it filed nowhere; the call is rejected instead when Archive is among the labels being removed and nothing else would remain, since removing it would delete the message
  - Parameters: `emailId` (required), `mailboxIds` (required array - each entry [id, role, name, or path](#naming-a-mailbox), resolved like every other mailbox tool; any unresolved or ambiguous entry rejects the whole call with the valid list)
...
`edit_draft` treats a signature already on a draft as a decision to preserve, not to re-request. Its `appendSignature` is a three-way parameter:

- Omitted - the signature is **kept**. If the draft carries a signature block this server wrote and the body you supply does not, the identity's *current* signature is re-appended and the result says so. This matters because editing `htmlBody` alone (the common edit) drops the unwritten text partner and replaces the whole HTML body, which is exactly where a signature would otherwise vanish silently. If the keep cannot be honoured, the result says *that*, rather than dropping the block in silence - and "cannot be honoured" is wider than it sounds: the address the edit sends as may have no signature configured (or not be a verified identity), **or** it may have one that is images-only HTML while the edit leaves the draft as plain text, which has nothing to carry it.
- `true` - add the signature to a draft that never had one.
- `false` - remove it. Your new body is stored exactly as written.

`appendSignature:true` reports a request that landed nowhere for **every** reason the compose tools do, including the two an edit reaches on its own: a body you supplied that already carries the sign-off, and an edit that writes no body at all (which leaves both bodies untouched, so there is nothing to append to). An omitted flag is preservation rather than a request, so those two stay silent there - neither is a loss.

Recognition is by the block this server wrote, marked with a `fm-mcp-signature` class. A class is HTML, so **preservation works on drafts with an HTML body only**. On a plain-text draft there is no marker to find, and an edit that rewrites the text loses the sign-off - *including one this server appended*, not just a hand-typed one. The recovery is `appendSignature:true` on that edit, and it is safe to repeat: a text body that already carries **either** form of that identity's block - the HTML-derived one or the configured plain-text one - is left alone, so a read-edit-write loop signs once rather than stacking. Both forms are checked because which one is sitting in the body depends on whether the call that put it there shipped HTML, so converting an HTML draft to plain text with `clearFields:['htmlBody']` and handing back the text the draft gave you is covered too.

Where the block sits is checked as carefully as which form it is. A reply or forward draft keeps its quoted original or forwarded-message block *in the body*, so the text you read back has the sign-off somewhere in the **middle** rather than at either end. The check works by subtraction: it cuts the body at the forwarded-message separator, dropping that line and everything below it, then looks for the sign-off as a run of **whole lines** in what is above. Nothing tries to interpret the `On <date>, <name> wrote:` attribution above a quote, which is the part that varies by client and by language; a dashed separator does not.
...
- **bulk_remove_labels**: Remove labels from multiple emails simultaneously. The last-label archive fallback is decided per message, so one batch can archive some and merely unlabel others; a rejection is not, and aborts the whole batch
  - Parameters: `emailIds` (required array), `mailboxIds` (required array - each entry [id, role, name, or path](#naming-a-mailbox), resolved like every other mailbox tool; any unresolved or ambiguous entry rejects the whole call with the valid list)
...
- **list_calendar_events**: List calendar events (core fields only — no participants for token efficiency), one entry per occurrence in the requested window ([#64](https://github.com/JonathanGodley/fastmail-mcp/issues/64), [#100](https://github.com/JonathanGodley/fastmail-mcp/issues/100)).
  ...
  - **A row can sit on the day immediately outside EITHER end of the window.** The client-side re-filter widens every designator-less value by ±14 hours on **both** edges rather than dropping it on a guess (below), so an all-day event on a neighbouring day survives — and so does a zone-stripped *timed* event on one. Which end you actually see follows the sign of the account's UTC offset, because that decides which neighbouring UTC day the server itself matched: measured, a UTC+10 account sees the day **before**, a UTC-4 account the day **after**. Nor is it bounded at one day — 14 hours of slack on an all-day `end` that is already the following midnight reaches close to two calendar days. What *is* bounded is the direction: it only ever adds a row, never removes one, so check each `start` against the window you asked for.
  ...
  - **A TOTAL discovery failure is an error, not an empty list.** If the CalDAV server fails to list the account's calendars at all, this raises rather than returning `[]` — an empty result to an availability question reads as "you are free", which is the dangerous way to be wrong. A `calendarId` that matches no calendar raises for the same reason, naming the calendars that do exist: the display name is matched exactly, so `"work"` for a calendar called `"Work"` is a plausible miss and used to come back as an empty day. An **empty or whitespace-only** `calendarId` is such a value, not a way to ask for every calendar — omit the parameter for that. It narrows what the call touches, so it fails closed rather than silently widening to the whole account.
- **get_calendar_event**: Get a specific calendar event by ID. Returns organizer and participants when available. Throws if the ID is not found. For a repeating event this returns the **series master**: `isRecurring` and `recurrenceRule` are set, and `start`/`end` are the series' original dates, **not** the next or nearest occurrence — use `list_calendar_events` with a window to get occurrence dates. The id is the series id, so `update_calendar_event`/`delete_calendar_event` on it act on every occurrence. One exception: where a record holds only overridden instances and no master at all, what comes back is one of those overrides and carries a `recurrenceId` — so a `recurrenceId` in this response means you are **not** looking at the master. This path never normalises times to UTC, so it is where a zone-stripped `start` is most likely — see [Times come back without their zone](#times-come-back-without-their-zone).
...
- **update_calendar_event**: Patch an existing calendar event. **Single (non-repeating) events only** — an id that resolves to a repeating event is rejected, and no parameter overrides that (see [Repeating events cannot be changed or deleted here](#repeating-events-cannot-be-changed-or-deleted-here)). Preserves all existing data (attendees, reminders, recurrence rules, etc.) not being changed. Omit a field to leave it unchanged, though a call that names no field and no `clearFields` at all is rejected; passing an empty or whitespace-only string for `title`, `description`, or `location` is rejected (it won't silently blank the property). To delete `description` or `location`, list them in `clearFields`. Floating times (no Z/offset) preserve the original timezone. A new `start`/`end` is checked against the value it will sit beside — see [Start/end agreement](#startend-agreement). WARNING: providing `participants` replaces ALL existing attendee data; `participants: []` removes all attendees (and the now-orphaned ORGANIZER). The same [text size limits](#calendar-text-size-limits) apply as on create.
  - Parameters: `eventId` (required), `title`, `description`, `start`, `end`, `location`, `participants` (array; each entry is `{email, name?}` **or a bare email-address string**, and the whole array may be sent as a JSON string — same rules and same index-naming rejections as on create), `clearFields` (array of `"description"`/`"location"` to delete)
...
**Omitting one bound clamps the window to 366 days.** `startDate` alone runs a year forward from it, `endDate` alone a year back, and the response says which range it searched in a trailing `Note:`. That bound exists because of recurrence expansion, not tidiness: with `expand` on, the window is the range the server *generates occurrences over*, so an open-ended default asked Fastmail to materialise every occurrence of every repeating event through 2099 — and `limit` bounds none of that work, since the whole response is parsed and sorted before the list is trimmed. A window whose bounds you name yourself is never clamped, however long it is. A bound that resolves past the ends of the four-digit-year range — the invented half, or one you named, since the local-day rule resolves your value through a zone and an offset alone can push `9999-12-31` over — saturates at that edge rather than running past it. So `startDate: "9999-12-30"` on its own is answered rather than erroring on an unrepresentable date, and a saturated bound **you** named is disclosed in the same trailing `Note:`. (`endDate` alone runs the other way and cannot saturate at that end.) Where saturation collapses a one-sided window to zero length, the call is rejected as `InvalidParams` rather than answered with nothing.

#### Times come back without their zone

A `start` or `end` in a calendar response that ends in `Z` is a UTC instant. One with **no `Z` and no offset** (`2026-04-20T10:00:00`) is a bare wall clock, and the event may well be stored against a named time zone that this read does not carry through: the parser takes only the property's value and drops its `TZID` parameter, so `DTSTART;TZID=Pacific/Auckland:20260420T100000` reaches you as digits with no zone attached. Read as local time on a UTC+10 account, that example is two hours wrong.

Where it applies:

| Call | Times carry a zone? |
| --- | --- |
| `list_calendar_events` **with** a `startDate`/`endDate` window | Mostly — Fastmail normalises expanded *occurrences* to UTC, so those arrive with `Z`. An **all-day** event still comes back as a bare date, measured, and so does any row the expansion did not touch |
| `list_calendar_events` with no window | No — the stored value is passed through as-is |
| `get_calendar_event` | No — it fetches a single resource with no window, so nothing normalises it |

So treat a designator-less value as "these digits on somebody's clock" rather than as local time, and prefer a windowed `list_calendar_events` when you need an instant you can compute with. Carrying the zone through to the response is [#139](https://github.com/JonathanGodley/fastmail-mcp/issues/139); the ordering of results already resolves these values internally rather than comparing the spellings, which is right whenever the event's real zone is the configured one and is never worse than a text sort — but it is a best-effort placement of a value whose zone was already lost, not a recovery of it. An event stored against a *third* zone sorts by the local reading of its wall clock, and `limit` slices that order.
...
- **Recurring events, on read**: `list_calendar_events` expands recurrences server-side when given a window, so it reports real occurrence dates; `get_calendar_event` fetches a single resource with no window and therefore always returns the series master at its original date. The client-side window check that backs up the server's filtering has no `TZID` left to resolve — the parser dropped it before the check runs ([#139](https://github.com/JonathanGodley/fastmail-mcp/issues/139)) — so it widens every designator-less value by ±14 hours, on BOTH edges, and a zone-carrying event just outside the window may survive it. The row you actually see survive is usually an **all-day** event on a neighbouring day, since a bare date has no zone either; which side of the window that is follows the sign of the account's UTC offset, and the reach is closer to two calendar days than one. Erring that way is deliberate — the alternative is dropping events the server correctly matched. Separately, a block that still carries its **`RRULE`** is never dropped by this filter whatever its dates say: an unexpanded master shows the series' original `DTSTART`, and judging that date would turn a wrongly-dated row into a missing one. One residual ambiguity comes from the server: an expanded series' **first** occurrence carries no `RECURRENCE-ID` (Fastmail marks only instances after the first), so where a window holds that occurrence and no sibling, the entry cannot be told apart from a one-off and `isRecurring` is left off rather than guessed at. `get_calendar_event` on the same `id` settles it.
