...
`position` is how you read past the `limit` caps (`search_emails`/`list_emails` cap at 100, `get_recent_emails` at 50). Contacts and calendar listings do not take it: passing one back would be rejected as an unknown parameter. `list_contacts`/`search_contacts` state their total the same way but never carry a `nextPosition`, so a total larger than the returned count means the rest is out of reach behind the hard cap. The calendar listings go over CalDAV rather than JMAP and carry no summary line at all: they return a bare JSON array, so `list_calendar_events` gives you no signal that its `limit` cut anything off.
...
- **list_calendar_events**: List calendar events (core fields only — no participants for token efficiency)
  - Parameters: `calendarId` (optional; the `id`/URL or the display name from `list_calendars`, omit to read every calendar), `startDate` (optional, ISO 8601), `endDate` (optional, ISO 8601), `limit` (default: 50, hard cap 500)
  - **Across several calendars, `limit` is not a top-N.** Calendars are read one at a time and reading stops as soon as `limit` events have been gathered; only then are they sorted by start time and trimmed. So with `calendarId` omitted, a later calendar can be skipped entirely and an earlier event in it will not appear. There is no summary line and no total to tell you that happened, so scope with `calendarId` and narrow with `startDate`/`endDate` rather than raising `limit`.
