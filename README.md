...
`position` is how you read past the `limit` caps (`search_emails`/`list_emails` cap at 100, `get_recent_emails` at 50). Contacts and calendar listings do not take it: passing one back would be rejected as an unknown parameter. `list_contacts`/`search_contacts` state their total the same way but never carry a `nextPosition`, so a total larger than the returned count means the rest is out of reach behind the hard cap. `list_calendar_events` goes over CalDAV rather than JMAP but states its total the same way, and its total counts every matching occurrence across every calendar queried, before `limit` trimmed the list — so a total larger than the returned count means the rest is out of reach and the window is what to narrow.
...
- **list_calendar_events**: List calendar events (core fields only — no participants for token efficiency), one entry per occurrence in the requested window ([#64](https://github.com/JonathanGodley/fastmail-mcp/issues/64), [#100](https://github.com/JonathanGodley/fastmail-mcp/issues/100)).
  ...
  - **Three fields say what a date is**, and the answer is never ambiguous: `recurrenceId` present means `start`/`end` **are** the in-window occurrence; `recurrenceRule` present (the raw `RRULE`, and it never appears alongside `recurrenceId`) means this is the series **master**, shown at its **original** start date, which may be years before the window; `isRecurring` is set in both cases. A one-off event carries none of the three.
  - **`limit` is a genuine earliest-N.** Every target calendar is queried, and only then is the combined set sorted by start time and trimmed — so a cross-calendar availability check can no longer lose whole calendars to an earlier one filling the quota.
  - **The response opens with a summary line** stating how many events matched in total (`Showing 50 of 137 results.`). When that total exceeds the number returned, `limit` cut the rest off; there is no paging, so narrow the window rather than raising `limit`. Expansion makes the cap easier to hit than it used to be.
  - **A discovery failure is an error, not an empty list.** If the CalDAV server fails to list the account's calendars, this raises rather than returning `[]` — an empty result to an availability question reads as "you are free", which is the dangerous way to be wrong.
...
- **delete_calendar_event**: Delete a calendar event. **Sends mail** if the event has attendees — the server emails each of them a cancellation (see [Calendar invitations send mail](#calendar-invitations-send-mail)).
  - Parameters: `eventId` (required)
...
`list_calendar_events`' `startDate` and `endDate` are read as strict ISO-8601 and nothing else, on the same rule as `create_calendar_event`'s `start`/`end` (see [Accepted date spellings](#accepted-date-spellings)): a date (`2027-03-01`) or a full datetime (`2027-03-01T09:00:00Z`, `2027-03-01T09:00:00+10:00`), with surrounding whitespace trimmed. `2027/03/01`, `"March 1 2027"` and a day its month does not have (`2027-02-30`) are all rejected naming the parameter, rather than guessed at or silently rolled into the next month — either would move the window off the days you asked about.

The window runs from `startDate` **inclusive** to `endDate` **exclusive**, and there is one deliberate asymmetry between them:

- A date-only **`startDate`** means 00:00:00 UTC that day, exactly as it does in `search_emails`.
- A date-only **`endDate`** covers the **whole of that day** — `2027-03-10` runs through to the following midnight UTC. So a single-day query is `startDate` and `endDate` on the *same* date. Under the midnight-UTC reading that `search_emails` uses, that call would be a zero-length window returning nothing at all, which reads as an empty day rather than as a mistake.
- A **full datetime** `endDate` is taken literally as the exclusive end, with no extension.

A `startDate` that is not before the resulting `endDate` is rejected as `InvalidParams`.

#### Calendar known limitations

- **Recurring events, on read**: `list_calendar_events` expands recurrences server-side when given a window, so it reports real occurrence dates; `get_calendar_event` fetches a single resource with no window and therefore always returns the series master at its original date. The client-side window check that backs up the server's filtering cannot resolve `TZID` names (no timezone database ships here), so a zone-carrying event just outside the window may survive it. That direction is deliberate — the alternative is dropping events the server correctly matched.
- **Recurring events, on write**: Only "all events" modification is supported (master VEVENT). "This event only" or "this and future events" are not supported. Changing start/end asks for `confirmRecurring: true` only when the new timing would **orphan** an exception override, one whose recurrence-id no longer lands on an occurrence. The rejection names the orphaned dates, and passing the flag prunes them to prevent server errors. Exceptions that still line up with the new rule need no flag, and neither does an event with no overrides. Where the recurrence rule cannot be expanded safely (a sub-daily frequency, or one this server cannot parse) nothing is pruned and no confirmation is asked for.
