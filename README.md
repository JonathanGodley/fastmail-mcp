...
- **Draft-first compose** — every message you compose is saved as a draft first, and **`send_draft` is the only tool that transmits a composed message**: `send_email` is removed, and `draft_email` only ever saves a draft, so every outgoing message exists as stored, inspectable bytes before transmission ([#32](https://github.com/JonathanGodley/fastmail-mcp/issues/32), [#66](https://github.com/JonathanGodley/fastmail-mcp/issues/66)). **One path outside compose still sends mail:** naming attendees on a calendar event makes the server email them an invitation, and a later delete emails a cancellation — see [Calendar invitations send mail](#calendar-invitations-send-mail) before gating on `send_draft` alone.
...
- **List parameters take a bare string too** - a single value, a comma-separated string or a JSON-encoded array all coerce to an array. That covers `to`/`cc`/`bcc`/`replyTo`, `emailIds`/`mailboxes`, `clearFields`, `removeAttachments`, `fields`, and `inReplyTo`/`references` on `draft_email`'s `mode:'new'`. Accepting those *shapes* is not the same as coercing away a value that has none of them — see the strict-reading bullet below for the parameters where an unreadable value is a refusal rather than a silent `undefined`.
  - **Every one of them now *declares* the string form in its schema** (`"type": ["array", "string"]`), so a client that validates arguments before sending passes the stringified form through instead of rejecting it and leaving the coercion unreachable. A client that does not validate is unaffected either way.
  - **Every one of them also *names the strings it reads* in its own description**, so which shapes are accepted is readable off the parameter rather than inferred from the type - which says only that *some* string is allowed.
  - **The accepted set is not the same for all of them.** The lists whose entries are OBJECTS - `attachments`, and `emails`/`phones`/`addresses` on the contact tools - read a whole-value string only as a **JSON-encoded array**, and reject anything else naming the parameter; the comma-separated and single-bare-value forms are not among their shapes. Each of those parameters says so outright, rather than leaving you to infer the refusal from a form its description happens not to mention. `participants` on the two calendar write tools is the same contract in its own words, with a worked example of the object shape.
- **Some parameters read their values and elements strictly, deliberately: `archive_email`'s `emailIds`, `search_emails`' `requiredMailboxes` and `excludeMailboxes`, and the four recipient fields (`to`/`cc`/`bcc`/`replyTo`) on `draft_email` and `edit_draft`.** The bare-string and comma-separated forms are still accepted, but a whole value that cannot be read as a list (a number, an object, a boolean) is rejected naming the parameter instead of coming back as "not supplied", and a non-string, empty or whitespace-only *element* is rejected by index instead of being coerced. The common thread is that a dropped or mangled value there cannot be told apart from a legitimate outcome: on `archive_email` it comes back as a `notFound` entry, which that tool documents as meaning the server did not know the id; on `search_emails` it silently widens or narrows which mailboxes the search touched; on `draft_email`'s `mode:'reply'` a dropped `to` reads as one you never passed, so the reply-all default fills `to`/`cc` from the original and carries its `Bcc` list with them; and on `edit_draft` a dropped field reads as "leave this one unchanged", so the draft keeps the recipients you were replacing and the edit reports success. On the recipient fields that also means `[""]` is now refused by index rather than shipped as one blank recipient. An empty or whitespace-only value for the *whole* parameter (`[]`, `""`, `" "`) is still read as the empty list rather than refused as a bad type — what an empty list then *means* is each tool's own rule, not a shared one: on `draft_email`'s `bcc` it reads as omitted (the reply carry still runs), while `edit_draft` refuses an empty recipient field outright and points you at `clearFields`. `emailIds` on `bulk_move`/`bulk_delete`, and `mailboxes` on the label tools, keep the lenient reading. Every array parameter, lenient or strict, **trims** its elements, so `["e1", " e2"]` and `"e1, e2"` mean the same thing - an untrimmed id would otherwise reach the server and come back as a not-found, which reads as a wrong id rather than as stray whitespace.
...
> **Draft-first compose:** there is no send-in-one-call tool. `draft_email` saves a draft in every mode, and **`send_draft` is the only tool that transmits a composed message** — so every outgoing message exists as stored, inspectable bytes before it is sent, and a name-based permission system can gate exactly one compose verb ([#32](https://github.com/JonathanGodley/fastmail-mcp/issues/32), [#66](https://github.com/JonathanGodley/fastmail-mcp/issues/66)). To compose and send: `draft_email` → `send_draft`. Gating `send_draft` does **not** gate all outbound mail: see [Calendar invitations send mail](#calendar-invitations-send-mail).
>
> **Recipient format:** every recipient field (`to`/`cc`/`bcc`/`replyTo` on `draft_email` and `edit_draft`) accepts each entry as either a bare address (`a@x.example`) or the RFC 5322 `"Name <email>"` form (`Alice <a@x.example>`), which is parsed into a display name + address. The SMTP envelope always uses the bare address. All four **fail closed**: a whole value that cannot be read as a list is refused naming the parameter, and a non-string or blank entry is refused by index, rather than either being dropped and read as a field you never passed (see [Parameter validation](#parameter-validation)). An empty or whitespace-only value for the whole parameter (`[]`, `""`, `" "`) is still read as the empty list rather than refused as a bad type; what an empty list then means is per-tool — `draft_email`'s `bcc` reads it as omitted, and `edit_draft` refuses an empty recipient field and points you at `clearFields`.
>
> **Draft sender name:** drafts created or edited via `draft_email`/`edit_draft` carry a display name alongside the From address, so the From shows your name rather than a bare address. You can state that name yourself by passing `from` in the `"Name <email>"` form on either tool ([#161](https://github.com/JonathanGodley/fastmail-mcp/issues/161)) — the address half is matched against your verified identities exactly as a bare address is, and the name half is written through without being validated, because nothing on the platform reads it. A name you pass wins outright; the rest of the chain below decides the name only when you passed none. That matters most under a wildcard identity, where one configured name otherwise stands behind every address in the domain. On an edit, behind that arm, the name is resolved in the *opposite* order from the address: the name the stored draft already carries against the address it writes beats the verified identity that owns that address, which is only a fallback for a draft that carries none — so a display name you set yourself is never silently reverted to your account's configured name by a later edit that didn't even touch `from` ([#152](https://github.com/JonathanGodley/fastmail-mcp/issues/152)). A stored `from` matching no verified identity (an address removed from the account, or a draft made elsewhere) keeps both its own address and its own display name (blank/whitespace-only counts as no name), rather than borrowing the default identity's name and putting it in front of a foreign address. See [`docs/email-bodies.md`](docs/email-bodies.md).
>
> **Wildcard identities are not addresses:** a Fastmail account can hold a *wildcard* identity, whose address is the pattern `*@example.com` rather than a real address. It verifies, and signs, any concrete address in that domain — but the pattern itself must never reach a From header, so **a pattern is refused wherever it would be used as an address** ([#160](https://github.com/JonathanGodley/fastmail-mcp/issues/160)). That covers all three directions the pattern can arrive from. **Passed as a `from` value** — `draft_email` or `edit_draft` with `from: "*@example.com"`, or the same pattern inside the `"Name <email>"` form — it is refused before anything is written, naming the address half; a passed pattern does *not* fall through to the "matches a verified identity" check, which would wave it through (the pattern equals the wildcard identity it came from). **Selected as the identity's own address** — `draft_email` with no `from` while the account default is a wildcard, or `edit_draft` with no `from` on a draft that carries none either — it is refused with a different message, since there the fix is to pass a `from` at all. And **already stored on a draft**, `send_draft` refuses to transmit it: the heal path for a draft written before these refusals existed, or by another client. **Everything else composes normally.** Passing a concrete `from` in that domain works, is verified by the wildcard identity and carries its signature; and a draft that already stores a concrete From edits normally, even when that address matches no identity at all — refusing there would make a draft composed under a wildcard identity permanently uneditable.
...
- **list_emails**: List recent emails across all mailboxes (or one, via `mailbox`). **Trash and Spam are excluded by default** (set `includeTrash`/`includeSpam` to include them); drafts are included (set `excludeDrafts` to omit them). When a Trash/Spam match is withheld, a trailing note reports how many — so no note means nothing filed only in Trash/Spam matched, and you need not re-search to check. This is not an arrivals feed: "all mailboxes" means every folder except Trash and Spam, so Sent copies, drafts and custom folders come back alongside newly received mail — pass `mailbox: "inbox"` for delivered mail only. One mailbox is all this tool scopes to; to intersect several mailboxes or exclude some, use `search_emails` with no query (see [Scoping across several mailboxes](#scoping-across-several-mailboxes)). Pass a small `limit` (e.g. 10) for a quick check of what has come in.
...
- **draft_email**: Compose a draft in one of three modes — `new`, `reply` or `forward` — placing this server's generated blocks yourself with **body tokens**. Three tokens are recognised in `textBody`/`htmlBody`: `{{signature}}` (the sending identity's configured signature), `{{quote}}` (the quoted original, `reply` only) and `{{forward}}` (the forwarded-message block, `forward` only). **Nothing is added for you** — a body with no `{{signature}}` is stored unsigned and the result says so, and a reply with no `{{quote}}` is stored without the conversation. Each mode accepts exactly its own history token and refuses the other; a near-miss spelling (`{{Quote}}`, `{{{quote}}}`) is refused rather than shipped as braces, while whitespace inside the braces is trimmed, so `{{ quote }}` is the token and a single brace each side (`{quote}`) is ordinary prose. The same token twice in one part is refused as well — expanding it twice would store the block twice. Tokens are expanded **once, at write**, so the stored draft is the finished message: `get_email` and the Fastmail web app both show what will be sent, and a body read back carries no token to re-expand. A block is substituted at the token's exact position with nothing added around it, so the spacing at the join is yours — the canonical shape is the sign-off above the history with one blank line (text) or one empty block element (HTML) between them. A token in one supplied part but not in the other supplied part is refused; supply one part and let the other be derived, or place the token in both. A body that expands to nothing visible is refused rather than stored. To write the braces literally, escape them with a backslash (`\{{signature}}`, which in JSON is `"\\{{signature}}"`); the backslash is consumed only before one of the three names. **A `reply` is reply-all by default** ([#184](https://github.com/JonathanGodley/fastmail-mcp/issues/184)): pass neither `to` nor `cc` and `to` becomes the original's `Reply-To` (falling back to its `From`) while `cc` carries everyone else the original's To and CC named — minus your own verified identities (wildcard identities included), minus whoever `to` already covers, deduplicated by address and omitted when nothing is left. **The original's own `Bcc` list is carried the same way** ([#189](https://github.com/JonathanGodley/fastmail-mcp/issues/189)): whole, in the original's order, display names kept, deduplicated by address — and with nothing dropped for being one of your identities or for already appearing in the reply's `to` or `cc`, because that is what pressing Reply in Fastmail stores. The carry keys on the original carrying a `Bcc` header and on nothing else — there is no provenance check. Mail you received normally carries none, so a reply to it usually carries nothing; an imported or migrated message that does carry one has its list carried like any other. **Naming either `to` or `cc` turns all of that off** and the recipients are wholly yours, because an explicit target means a deliberately narrowed reply, and carrying a *blind* list behind a narrowed reply is the one outcome nobody could mean; an absent `to` still falls back to `Reply-To`/`From`. Naming `bcc` is different: it is additive rather than narrowing, so it replaces the carried Bcc alone and leaves the `to` and `cc` defaults running (an empty `bcc` — `[]` or `""` — is treated as omitted). The result reports the `to`, `cc` and `bcc` actually stored, and says when the Bcc list was carried. **Always saves a draft, never transmits** — review it, then send it with `send_draft`, which on `reply`/`forward` marks exactly the stored copy passed as `originalEmailId`. Quoted and forwarded HTML is reproduced **sanitised** and re-sent under your From address, and **images the original displayed are carried into the block** on a bound this server does not narrow — a quoted image is only carried when the HTML part is non-blank **and** carries the history token, otherwise the block ships as text and the result says how many images went with it — dropped on a reply, riding as regular attachments on a forward, where the result also says the original ships HTML and that putting `{{forward}}` in `htmlBody` keeps the formatting and the images both. The result carries a **receipt**: which tokens expanded in which part, how many times, in what order they landed, which were removed for having nothing to expand to (and why), any `{{…}}` spelling in your body it left unexpanded (each distinct spelling once, however many times and in however many parts you wrote it), and whether the `asAttachment` filler body was inserted. (This tool returns a status string plus the receipt, not email data, so `raw`/simplification do not apply.)
  - Parameters: `mode` (**required**, one of `new`/`reply`/`forward`, no default), `originalEmailId` (required for `reply`/`forward`, refused on `new`), `to` (required on `forward`; optional on `reply`, where it defaults to the original's `Reply-To` else its `From` — and naming it turns the reply-all carry off; optional array or a single bare string), `cc` (optional; on `reply` it defaults to the reply-all carry above, and naming it also turns that carry off), `bcc` (optional; on `reply` it defaults to the original's own Bcc list carried whole, and naming it replaces that list without touching the `to` and `cc` defaults), `from` (optional — a bare address or the `"Name <email>"` form, where the address half must match a verified identity and the name half is written through unvalidated; omit the name and the matched identity's name is written. Also picks, by that address half, the identity whose signature `{{signature}}` expands to; required when the account's default identity is a [wildcard](#email-tools), and the wildcard *pattern* itself is refused as a value here — pass a concrete address in its domain), `replyTo` (optional), `mailbox` (`new` only — [id, role, name, or path](#naming-a-mailbox); defaults to the Drafts folder, resolved by its `drafts` role, so an account with no such mailbox is refused rather than guessed at by name; **naming anywhere but Drafts makes the draft unsendable until it is moved back** — the draft is filed into that mailbox and that mailbox only, and `send_draft` sends only a draft that is in Drafts, so use this for a draft you are parking, not one you are about to send), `subject` (optional; `reply` inherits `Re: …` and `forward` inherits `Fwd: …`, neither double-prefixing; typing a reply or forward prefix (`Re:`, `Fwd:`, `Fw:`, with or without an `[n]` counter) into a `new` subject draws a note, since `new` writes no threading headers — unless you pass `inReplyTo`/`references` yourself), `textBody` (optional), `htmlBody` (optional — supplying it non-blank and carrying the history token is what lets the block show the original's images), `inReplyTo` (`new` only; array or a single bare string), `references` (`new` only; array or a single bare string), `asAttachment` (`forward` only — refuses `{{forward}}` alongside it, since the original is already carried whole; a forward with **neither** `{{forward}}` nor `asAttachment` is refused), `includeOriginalAttachments` (`forward` only, default true), `attachments` (optional).
- **edit_draft**: Edit an existing draft email. Since JMAP emails are immutable, this creates a replacement draft and moves the old one to Trash (so the returned email ID is new). The edit preserves the draft's threading headers (In-Reply-To/References), attachments, and other keywords. The replaced draft is **never destroyed** — it stays in Trash until Trash is emptied or auto-purged (Fastmail's Trash retention is a per-account setting, and can be set to never), so an edit made from a stale copy (the draft changed in the web UI or another client in between) can be undone. The result also reports what the replaced draft held (subject, recipients, body sizes); compare that against what you meant to replace to spot an unintended overwrite ([#65](https://github.com/JonathanGodley/fastmail-mcp/issues/65)). On the rare failure where the replacement is created but the old copy can't be moved to Trash, you are left with a duplicate draft rather than none, and the result says so. Only fields you provide are changed; omit a field to leave it unchanged. **The body you supply is stored exactly as written** — nothing is appended, removed or rebuilt in it, and the one exception is opt-in and explicit (`expandSignature: true`, below) — and any edit that writes or clears a body must pass `bodyHash` from a `get_email` read of that draft.
  ...
  - Parameters: `emailId` (required), `to` (optional array), `cc` (optional array), `bcc` (optional array), `from` (optional — a bare address or the `"Name <email>"` form; a name written here **replaces** the draft's stored display name, while a bare address keeps it (switching to a different address you can send as writes that identity's name instead, since the draft carries no name for the new address), and a name can never be *removed*, only replaced. Omitting `from` keeps the draft's own From, including one written under a [wildcard identity](#email-tools); it is required only on a draft that carries no From while the account default is a wildcard, and the wildcard *pattern* itself is refused as a value here — pass a concrete address in its domain), `subject` (optional; writing a reply or forward prefix (`Re:`, `Fwd:`, `Fw:`, with or without an `[n]` counter) onto a draft that carries no threading headers draws a note, since an edit cannot add them), `textBody` (optional), `htmlBody` (optional), `replyTo` (optional array), `bodyHash` (**required for any edit that writes or clears a body**, below), `expandSignature` (optional boolean, default false — expands a `{{signature}}` you wrote, below), `clearFields` (optional array), `attachments` (optional array — **appends**), `removeAttachments` (optional array of blobId/name)
  ...
  - **Proving your read: `bodyHash`.** Because an edit replaces the body wholesale, one written from a stale read silently overwrites whatever changed in between. So every edit that writes `textBody`/`htmlBody` or clears one of them must pass `bodyHash` — the value `get_email` returns for that draft. A missing or stale hash is **rejected**; re-read the draft and edit from that. It is a lost-update guard and nothing more: it proves you *saw* the body, never that you kept any of it, so it passes on an edit that replaces the body entirely. A metadata-only edit is body-invariant and needs none — but an attachment edit can still **stale a hash you are holding**, because the hash covers the parts the draft's **body lists** carry. `removeAttachments` taking off a part the server had routed into a body list (an embedded image, typically) changes what the hash is taken over with no body written; taking off a part that only ever sat in the attachments list changes nothing the hash covers. **Adding** one may go the same way if the server routes the new part into a body list — this server always writes attachments to the attachments list and does not decide where a later read surfaces them, so treat that direction as unconfirmed. Nothing is lost either way: your next *body* edit is rejected and you re-read. A successful body edit hands the **next** hash back in its result, read off the saved draft, so a run of edits costs one `get_email` at the start rather than one between each pair. When a hash cannot be issued honestly the result says which case it hit and you re-read: the expansion rewrote the body, the surviving body is one this server derived from HTML rather than one you supplied (clearing `htmlBody` on a draft that never had one leaves your own stored text standing, so that edit *does* get a hash), or the read-back failed. There is a fourth group, and it is the one that does not end in a re-read. The saved draft is judged by exactly the rule `get_email` applies to any draft — one rule, so the two tools never disagree about the same stored body — and where that rule says a hash could never be spent, neither tool issues one: a part the server flagged as truncated or with an encoding problem, a body part no read returns at all (its declared type doesn't match the body list it sits in), or a body that puts two parts of the same text type in one list — a layout every edit is refused over, so the token would have nothing to buy ([#180](https://github.com/JonathanGodley/fastmail-mcp/issues/180)). Those notes say to **recreate** the draft. A returned hash always comes from re-reading the saved message — never from the bytes the call sent.
  - **Expanding a signature: `expandSignature`.** Off by default, and off is what makes the byte-for-byte promise hold: nothing this server finds in a body can make it rewrite one, which matters because part of a draft body you hand back was written by someone else (a quoted original, a forwarded message) and could carry a planted placeholder. Pass `expandSignature: true` and a `{{signature}}` you wrote is replaced with the sending identity's configured sign-off; see [Signing a message](#signing-a-message). `{{quote}}` and `{{forward}}` never expand here and are stored as the literal text you typed — this tool never places history for you. Passing the flag with no `{{signature}}` in the body is rejected, as is the same token twice in one part. A near-miss spelling (`{{Signature}}`, `{{{signature}}}`) is stored as written and **reported** rather than refused, since it may be text the original's author wrote. **A `{{…}}` naming no token at all (`{{sig}}`) is reported too**, on flagged and unflagged edits alike, because this tool stores what you wrote and a mistyped token ships with its braces showing. A `{{signature}}` you store on an *unflagged* edit is reported the same way, and is how you keep one as literal text. **Every one of these notes is count-rise**: a `{{signature}}`, a near-miss, a `{{quote}}`/`{{forward}}`, an escaped spelling or a `{{…}}` naming no token that the draft's **stored** body already carried passes in **silence**, and only what *this* edit added is named. **A note that names a spelling rather than a part is said once for the whole edit**, however many times you wrote it and whether it went into one supplied body or both — one typo is one thing to fix. That is what stops a token sitting in a quoted original — text the original's author wrote, which you cannot change without rewriting their words — being reported back to you on every edit of that draft for as long as it exists. To write the braces as text *under the flag*, escape them: `\{{signature}}` (in JSON, `"\\{{signature}}"`). The backslash is consumed and the bare token is stored, so it will not expand again unless a later edit passes the flag too. On an unflagged edit there is nothing to escape — that body is stored byte for byte, backslash included — and the result says so when *this* edit added the escape, because a backslash you meant as an escape has just shipped as content.
- **send_draft**: Send an existing draft email — **the only tool that transmits a composed message** (see the draft-first note above; calendar invitations are sent by the server, outside compose — see [Calendar invitations send mail](#calendar-invitations-send-mail)). The draft must have recipients and a from address, and that address must be a real one: a draft whose stored From is a **[wildcard pattern](#email-tools)** (`*@example.com`) is refused, because the asterisk would go out as the sender — edit the draft with an explicit `from` first. **It must also be in the Drafts folder** — a draft moved to Archive, to Trash or to any other folder is **rejected**, and the refusal says where it is now; move it back to Drafts with `move_email` and send it again. Being in Drafts is what says the message is still meant to go out: filed anywhere else, it has since been made something other than outbound mail, and this is the only tool that transmits. A draft that is in Drafts *and* carries another label is still in Drafts and sends normally, and **keeps that label**: the send files the message into Sent by **patching** `mailboxIds` — naming just those two mailboxes, removing Drafts and adding Sent — so every other label the draft carried, and any Inbox membership, survives the send untouched. `delete_email` does **not** work that way: its move to Trash writes `mailboxIds` whole-value, replacing the entire membership, so it does drop a message's other labels and the Trash copy cannot show you what they were — [#123](https://github.com/JonathanGodley/fastmail-mcp/issues/123) is the open record of that one. Both the Drafts folder it must be in and the Sent folder it is filed into are resolved by their **role**, not by name, so a custom folder called "Draft notes" or one called "Sent" satisfies neither; if no mailbox in the account carries the `drafts` role, or none carries the `sent` role, `send_draft` refuses rather than sending unchecked or sending with nowhere to file the sent copy. Both refusals come **before** the submission, so nothing is transmitted. A third refusal covers the case where the server returns the draft with **no readable filing at all** — it carries its own message, names no location and suggests no move, because the draft's whereabouts are unknown rather than known to be wrong. An **HTML-only draft with real content** sends as-is — HTML-only mail is valid. That is a draft whose HTML yields no derivable text at all, e.g. one showing a remote image with no alt text; a draft displaying an embedded (`cid:`) image is not in that group, since it derives `[image]` and carries a text part. Only a **genuinely empty body part** (e.g. a blank `htmlBody` alongside real text, which can happen for drafts created in other clients) is **rejected**: an empty `text/html` part renders blank and shadows a real `text/plain`, so the recipient would see nothing. Edit the draft to supply or clear that body first. **Every** body part is checked, so a blank one is rejected wherever it sits in the draft's body list and whether or not it declares a content type — a part that declares none counts as the body of whichever list carries it, which is how a recipient's client renders it, so a blank one renders blank just the same. (Drafts created by this server never carry an empty part; every send/draft path drops empty bodies on write.)
...
- **add_labels**: Add labels (mailboxes) to an email without removing existing ones. Labels means the **Inbox and the account's own labels**; a folder (any other role mailbox) is rejected - use `move_email`. Adding the inbox label is how a message is put back in the Inbox
  - Parameters: `emailId` (required), `mailboxes` (required array - each entry [id, role, name, or path](#naming-a-mailbox), resolved like every other mailbox tool; any unresolved or ambiguous entry rejects the whole call with the valid list, as does any entry that resolves to a folder rather than a label)
...
`draft_email` takes an optional `subject` in every mode. Omit it and the reply inherits `Re: <original subject>` (no double-prefixing); pass it and your line is used verbatim, with no `Re:` added. A blank string is treated as omitted; a non-string is rejected. On `mode:'forward'` it behaves identically over the `Fwd:` default, so the two history modes do not differ on the same parameter. The `In-Reply-To`/`References` headers are built the same way either way, so **recipients thread the reply correctly whatever subject you choose**.
...
- **Hand-rolling `inReplyTo`/`references` on `mode:'new'` under a mismatched subject is worse, and is not undoable.** Once such a draft exists, later drafts replying to that *same original message* are grouped onto the splinter thread too, including ones created afterwards with the correct `Re:` subject and the full reference chain. Deleting the offending drafts, emptying them from Trash, and recreating with a trimmed reference list all leave the mapping in place ([#68](https://github.com/JonathanGodley/fastmail-mcp/issues/68)).

So reply with `mode:'reply'`, with or without `subject`, rather than reconstructing a reply out of `mode:'new'` plus manual threading headers. Typing the prefix yourself is the half-way version of that mistake, and both compose and `edit_draft` now say so: a `Re:`/`Fwd:`/`Fw:` prefix written into a `mode:'new'` subject, or edited onto a draft that carries no threading headers, draws a note naming the mode that would have threaded it ([#188](https://github.com/JonathanGodley/fastmail-mcp/issues/188)). It is a note and never a refusal — a fresh conversation reusing an old subject is legitimate — and each route has its own silencer: on compose, passing `inReplyTo`/`references` yourself; on `edit_draft`, a draft that already carries `In-Reply-To`, `References` or `X-Forwarded-Message-Id`.
...
The sending identity's configured signature - the sign-off you set in Fastmail - is not applied by JMAP, so nothing signs a message unless you ask. In `draft_email` you ask by writing `{{signature}}` into `textBody`/`htmlBody` at the point the sign-off belongs; `edit_draft` expands the same token behind `expandSignature: true`. Nothing is added anywhere you did not put it. The canonical placement is **above** the quoted original or the forwarded-message block, where a sign-off belongs; below one reads as part of the quote ([#33](https://github.com/JonathanGodley/fastmail-mcp/issues/33)).

The token is expanded **once, at write**, so the saved draft is the finished message: `get_email` and the Fastmail web app both show the sign-off that will be sent, and a body read back carries no token to expand again.
...
A body with **no** `{{signature}}` in it is stored unsigned, and the receipt says which tokens expanded and which never appeared - so "this draft is unsigned" is never something you have to re-read the draft to find out.
...
The flag exists there rather than the token alone because part of a draft body handed back to an edit was written by someone else — the quoted original, the forwarded message — so an in-band trigger could be **planted** by them. A flag cannot be. `draft_email` needs no such flag for the same reason turned around: it expands only the body **you** authored, before any quoted or forwarded content is joined in, so a token sitting inside somebody else's message is never one this server expands. The consequence for editing is the useful one: with the flag absent, a `{{signature}}` sitting in a stored body is stable under every edit. What the result reports is what *you* added — a token the body you hand back carries more of than the stored body did is named in a note; one that was already in the stored body passes in **silence**, because reporting it would mean reporting the original author's text back to you on every edit of that draft for as long as it exists.
...
The same pair works in every `draft_email` mode and on `edit_draft`. What you can rely on:
...
- The plain-text alternative derived from your HTML shows an image's alt text, and `[image]` for an **embedded** (`cid:`) image that has none — so a message of embedded pictures still reaches text-only readers. A **remote** (`http(s)`) image with no alt contributes nothing at all, so a body that is nothing but remote images reaches those readers empty. Give remote images alt text.
...
- **list_calendars**: List all calendars. Each entry carries `id`, `displayName`, `url`, and `description`/`color` when set. A calendar's `id` **is** its CalDAV URL, and every `calendarId` parameter accepts either that or the calendar's display name. Prefer the `id` when it matters: a display name is not unique per account, and one that matches two calendars is refused everywhere it is accepted ([#173](https://github.com/JonathanGodley/fastmail-mcp/issues/173)).
  - **A collection the server failed to describe is named, not dropped** ([#136](https://github.com/JonathanGodley/fastmail-mcp/issues/136)). When an entry inside the account's calendar home comes back broken - a failed response element, or a failed `propstat` that strips its properties - it cannot appear as a row, because the failure destroys its display name and its `resourcetype` together. The response instead carries a trailing `Note:` line giving its path, and the subject counts them - the line *opens* `Note: a collection in the calendar list failed to list:` for one and `Note: 3 collections in the calendar list failed to list:` for several, then runs on past the path with what that failure does and does not mean. If you are matching on it, match the `in the calendar list failed to list` phrase rather than the singular sentence. **The note does not claim it was a calendar**, and neither should you: nothing survives the failure to say what kind of collection it was, so the honest statement is that a collection could not be listed and this listing is the ones that did answer. The same note appears on `list_calendar_events` and `get_calendar_event`; `create_calendar_event` refuses it as a target; `update_calendar_event`/`delete_calendar_event` act on the copy they found and name what they could not look in.
  - **Detection happens when the list is built, and a list built while one was broken is never cached.** So the next calendar call re-asks the server, and the note goes away by itself once the collection recovers. The residual, stated plainly: a *healthy* list **is** cached for the life of the process, so a calendar that breaks **after** a successful discovery keeps being listed here until the server restarts.
- **list_calendar_events**: List calendar events (core fields only — no participants for token efficiency), one entry per occurrence in the requested window ([#64](https://github.com/JonathanGodley/fastmail-mcp/issues/64), [#100](https://github.com/JonathanGodley/fastmail-mcp/issues/100)).
  ...
  - **Four fields say what a date is**: `recurrenceId` present means `start`/`end` **are** the in-window occurrence; `recurrenceRule` present (the raw `RRULE`) means this is the series **master**, shown at its **original** start date, which may be years before the window; `recurrenceDates` present (the raw `RDATE` values, comma-separated) says the same for a series that **lists** its occurrences instead of stating a rule; `isRecurring` says the entry belongs to a repeating series, and on a row read out of a VEVENT its absence says it does **not** - the field is decided there rather than omitted when the evidence is thin. One kind of row makes no recurrence claim in either direction: a stored record this server can read no VEVENT out of still produces a row, titled `Untitled` and carrying no dates, and `isRecurring` is absent on it because nothing was established about it. **`recurrenceDates` carries the values and not the parameters** - a `TZID`, `VALUE=DATE` or `VALUE=PERIOD` on the `RDATE` line is dropped - so a designator-less value in that list does **not** follow the `timeZone` rule that governs `start`, and RDATEs written in different zones are indistinguishable once joined into one list. It is proof that other dates exist, not a date you can place on a clock; pass a window and let the server expand the series if you need real occurrence times. Master and occurrence are the usual split, but the two fields *can* appear together — RFC 5545 lets an override block carry its own rule — and where they do, `recurrenceId` names the instance and `recurrenceRule` is that block's own rule rather than the series'. There is one case the expanded output cannot distinguish, and it is the server's doing rather than a choice made here: Fastmail sets a `RECURRENCE-ID` only on instances **after** the first, so a series' first occurrence arrives bare, and a window holding **only** that occurrence yields a block indistinguishable from a one-off. Those blocks are settled before the listing returns, by re-reading the stored records they came from - a stored record still states the repeat the expansion stripped - so `isRecurring` is trustworthy on every row a VEVENT was read for. The extra read is one request per calendar rather than one per row, and is skipped entirely for a calendar with nothing ambiguous in it - but note that a single markerless block is exactly what an ordinary one-off looks like, so on a normal listing nearly every row is ambiguous and its stored payload is fetched a second time. If the read cannot be completed, or answers with a payload holding no readable event, the call **fails** rather than return rows whose recurrence it could not establish ([#155](https://github.com/JonathanGodley/fastmail-mcp/issues/155)).
  ...
  - **`transparency` appears on a row only when the event is not busy** — normally `"free"`, or a non-spec token where the record holds one. Most events block your calendar, so a row that says nothing is busy and the field is omitted to keep rows small. `get_calendar_event` states it on every event instead — see [Busy and free](#busy-and-free-an-all-day-event-is-free-a-timed-event-busy).
  ...
  - **A TOTAL discovery failure is an error, not an empty list; a PARTIAL one is named.** If the CalDAV server fails to list the account's calendars at all, this raises rather than returning `[]` — an empty result to an availability question reads as "you are free", which is the dangerous way to be wrong. A partial failure has three forms and they behave differently ([#136](https://github.com/JonathanGodley/fastmail-mcp/issues/136)):
    1. **Broken inside the calendar-home listing** — detected when the list is built, from the same PROPFIND answer the list came from. This call answers from the calendars that did list and adds a trailing `Note:` line containing `in the calendar list failed to list` (subject `a collection`, or `N collections`) and naming its path. If that collection *was* a calendar, its events are missing from these results **and from the total**, so a quiet answer is not proof of a free day. Nothing is cached while one is outstanding.
    2. **Listed, then failed on read** — a calendar that answered for itself and then failed when its events were fetched fails the **whole call** with an error. There is no "rest of it" to answer from, so no partial listing is returned. Unchanged, and deliberately so.
    3. **Not visible to this account at all** — the server omits it from the listing entirely, so nothing on this side can report it.
  ...
  - **A `calendarId` whose display NAME matches more than one calendar raises too** ([#173](https://github.com/JonathanGodley/fastmail-mcp/issues/173)). A display name is not unique per account — a shared calendar is named by whoever owns it — and this call used to read **every** calendar the name matched, so "what is on my Work calendar" answered from two calendars at once and named neither. It now refuses, naming each match with its URL, and `create_calendar_event` refuses the same value the same way rather than silently writing into the first match. **The URL is the way through**: a calendar's `id`/URL addresses exactly one collection, so the URL form is never ambiguous — not even where another calendar's *display name* is spelled as that URL, which is a decoy anyone sharing a calendar with the account could plant. Same distinction, same reason, as an ambiguous `eventId` (see [One `eventId` can name more than one record](#one-eventid-can-name-more-than-one-record)).
  - **That URL rule also fixed a write redirect, which is the sharper half.** Both paths used to test URL-or-name in one predicate, so **discovery order** decided rather than any preference for an address. With such a decoy listed first, `create_calendar_event` aimed at a calendar's exact URL matched the decoy *by name* and wrote the event into the decoy's calendar, reported as a success naming the calendar you asked for. The URL is now resolved in its own pass, before any name is compared. Relatedly, a calendar the server describes without a usable URL can no longer be addressed by an **empty** `calendarId`.
- **get_calendar_event**: Get a specific calendar event by ID. Returns organizer and participants when available. Throws if the ID is not found. For a repeating event this returns the **series master**: `isRecurring` is set along with `recurrenceRule` (the raw `RRULE`) and/or `recurrenceDates` (the raw `RDATE` values, for a series that lists its occurrences rather than stating a rule), and `start`/`end` are the series' original dates, **not** the next or nearest occurrence — use `list_calendar_events` with a window to get occurrence dates. This unexpanded path is where those fields actually show up: Fastmail strips them from an expanded block, and both halves are measured - `calendar-expand.probe.mjs` for `RRULE`, `calendar-rdate-expand.probe.mjs` for `RDATE`. `recurrenceDates` carries the RDATE **values only** - a `TZID`, `VALUE=DATE` or `VALUE=PERIOD` parameter on the line is dropped - so a designator-less value there does **not** follow the `timeZone` rule that governs `start`, and it is evidence that other dates exist rather than a date you can place on a clock. **And passing a window will not place it on one either, for a series that states no `RRULE`**: Fastmail's window filter matches such a record over its `DTSTART` alone, so a `list_calendar_events` window covering one of these `RDATE` values but not the series start returns nothing at all for it, with no sign in the response that anything was left out ([#167](https://github.com/JonathanGodley/fastmail-mcp/issues/167)). Widen the window until it reaches the series start - the `start` in this response - or confirm the date in the Fastmail web interface. The id is the series id, so `update_calendar_event`/`delete_calendar_event` on it act on every occurrence. One exception: where a record holds only overridden instances and no master at all, what comes back is one of those overrides and carries a `recurrenceId` — so a `recurrenceId` in this response means you are **not** looking at the master. This path returns the stored `DTSTART`/`DTEND` exactly as written, with `timeZone`/`endTimeZone` naming the zone when it isn't the account's configured one — see [Calendar times: a name, never an offset](#calendar-times-a-name-never-an-offset).
  ...
  - **`transparency` is always present here**, as `"busy"` or `"free"` — or, where the record holds a token the spec does not define, that token verbatim. An event whose record carries no `TRANSP` is reported as `"busy"`, **derived** from RFC 5545 §3.8.2.7 rather than read off the record; the event is busy either way. `list_calendar_events` omits the field for every busy event instead. See [Busy and free](#busy-and-free-an-all-day-event-is-free-a-timed-event-busy).
- **create_calendar_event**: Create a new calendar event. Supports date-only (e.g. `2026-04-01`) for all-day events. DTEND is exclusive per RFC 5545 — a one-day event on April 1 needs `end: "2026-04-02"`. An all-day event is created **free** and a timed event **busy**, matching the Fastmail client; pass `transparency` to say otherwise — see [Busy and free](#busy-and-free-an-all-day-event-is-free-a-timed-event-busy). See [Start/end agreement](#startend-agreement) for the form and ordering rules, [Writing calendar times](#writing-calendar-times) for `timeZone`, and [Calendar text size limits](#calendar-text-size-limits) for the size caps.
  - Parameters: `calendarId` (required; the `id`/URL or the display name from `list_calendars`, matched case-sensitively and resolved by **literally the same function** as on a read — surrounding whitespace is ignored on both sides, and `list_calendars` reports the trimmed name, so the name it gives you is one this parameter resolves. A calendar `list_calendars` does not show cannot be written to either, and a name matching **more than one** calendar is refused here exactly as it is on a read, naming each match with its URL rather than writing into the first: pass that calendar's URL, which addresses one collection and cannot be ambiguous), `title` (required), `description` (optional), `start` (required, ISO 8601 or date-only), `end` (required, ISO 8601 or date-only), `location` (optional), `timeZone` (optional; IANA zone name for a designator-less `start`/`end` — omitted writes the account's configured zone, see [Writing calendar times](#writing-calendar-times)), `transparency` (optional; `"busy"` or `"free"` — omit to take the frame's default, all-day free and timed busy), `participants` (optional array; each entry is `{email, name?}` **or a bare email-address string**. The whole array may also be sent as a JSON string; a comma-joined string is *not* accepted. An unknown per-item key, a missing/non-string `email`, or a non-string `name` is rejected naming its position, e.g. `participants[2]`)
- **update_calendar_event**: Patch an existing calendar event. **Single (non-repeating) events only** — an id that resolves to a repeating event is rejected, and no parameter overrides that (see [Repeating events cannot be changed or deleted here](#repeating-events-cannot-be-changed-or-deleted-here)). Preserves all existing data (attendees, reminders, recurrence rules, etc.) not being changed. Omit a field to leave it unchanged, though a call that names no field and no `clearFields` at all is rejected; passing an empty or whitespace-only string for `title`, `description`, or `location` is rejected (it won't silently blank the property). To delete `description`, `location` or the event's statement about free/busy, list them in `clearFields`. Floating times (no Z/offset) preserve the original timezone. A new `start`/`end` is checked against the value it will sit beside — see [Start/end agreement](#startend-agreement). `timeZone` works differently here than on create — see [Writing calendar times](#writing-calendar-times). `transparency` sets or replaces the event's free/busy and `clearFields: ["transparency"]` deletes the property; with neither, an update never changes free/busy at all — not even when it converts the event between all-day and timed hours — see [Busy and free](#busy-and-free-an-all-day-event-is-free-a-timed-event-busy). WARNING: providing `participants` replaces ALL existing attendee data; `participants: []` removes all attendees (and the now-orphaned ORGANIZER). The same [text size limits](#calendar-text-size-limits) apply as on create.
  - Parameters: `eventId` (required), `title`, `description`, `start`, `end`, `location`, `timeZone` (optional; IANA zone name for a designator-less `start`/`end` you are also passing this call — never defaults when omitted, see [Writing calendar times](#writing-calendar-times)), `participants` (array; each entry is `{email, name?}` **or a bare email-address string**, and the whole array may be sent as a JSON string — same rules and same index-naming rejections as on create), `transparency` (`"busy"` or `"free"`; sets or replaces the event's free/busy, and conflicts with naming it in `clearFields`), `clearFields` (array of `"description"`/`"location"`/`"transparency"` to delete — clearing `transparency` removes the property, which leaves the event reading as busy without saying so)
...
#### What an `eventId` resolves to

`get_calendar_event`, `update_calendar_event` and `delete_calendar_event` all resolve an id through one lookup, so these rules hold identically on the read and on both writes ([#137](https://github.com/JonathanGodley/fastmail-mcp/issues/137)).

- **Two forms, and both are always tried.** An `eventId` may be the event's `id` (its `UID`) or the `url` from a `list_calendar_events` row. The lookup never picks between them by the *look* of the value: it asks every calendar for that UID **and** resolves the url form, then unions what comes back. UIDs are written by whoever created the event - anyone who can send this account an invitation - so a UID can perfectly well be URL-shaped, and dispatching on shape would make such an event unfindable by its own id.
- **The UID is asked of the server, not filtered out of a download.** Each calendar is sent one CalDAV `calendar-query` filtered on the UID, rather than having every event in it fetched and compared here. If the server refuses that query, **the call fails** - there is deliberately no quiet fallback to a full scan, because a lookup that fell back would be answering from a different set of records than the one it says it searched.
- **A url is resolved against this account's own calendars, and nothing else.** The url form is matched against the collections discovery already found - same host, and a path sitting underneath one of them on a path-segment boundary - and only then fetched, addressed to that known collection. A url pointing anywhere else is never requested at all; it simply names no record here and comes back as not-found. See [Path-confinement security model](docs/security-model.md) for why the caller's string is never itself a request target.
- **An empty or whitespace-only `eventId` is rejected** rather than searched for, so `"   "` gets a message about the argument instead of an answer about your calendars.
- **A record the server describes incompletely does not resolve.** A resource that comes back with no payload, with nothing parseable in it, or with no entity tag to write against, is treated as no match and reads as not-found. The entity tag is the `If-Match` of a write, and a delete sent without one destroys the resource unconditionally, whatever it has become since it was read - so this server will not change or destroy a record it could not read whole.
- **What a resource is NAMED does not affect whether it is visible** ([#191](https://github.com/JonathanGodley/fastmail-mcp/issues/191)). A calendar resource stored as `.ICS`, with no extension at all, or as a bare UUID is listed, read, updated and deleted exactly like a `.ics` one — the same rule on `list_calendar_events` and on all three id-resolving tools, so a lookup can never resolve a record the listing does not show. What keeps non-event resources out is the `VEVENT` filter the server is asked for, not the spelling of the href.
...
Two `TZID`-bearing values in *different* zones — a flight that departs in one zone and lands in another — are a legal shape and are accepted, and **their order is judged on instants, not on their wall clocks** ([#140](https://github.com/JonathanGodley/fastmail-mcp/issues/140)). **This shape only ever arises on `update_calendar_event`**, editing a record that already holds it: `create_calendar_event` takes no per-value zone and writes a designator-less pair in the configured zone, so both of its ends are always in one zone. The text of a cross-zone pair says nothing about its order in either direction: Rome 10:00 to New York 08:00 the same day is a real four-hour flight and is written, while New York 07:00 to Rome 08:00 ends four hours before it starts and is rejected with the same "end after start" message as any other backwards pair. The one case that still stands down is a `TZID` this server cannot resolve at all — a vendor name such as `AUS Eastern Standard Time`, which real records do carry — because there is then no instant to compare and refusing would reject a record your account already holds. Both rejections are `InvalidParams` and name both values and both forms.
...
`endTimeZone` describes `end`, but only **relative to `start`**: omitted whenever `end`'s zone matches `start`'s (the ordinary case), and otherwise set by the same rule as the table above. When `start` is absent entirely — no `DTSTART` at all — `end` is compared against the account's configured zone instead, the same fallback `timeZone` itself uses. RFC 5545 §3.8.5.3 and this server's own write-side check ([Start/end agreement](#startend-agreement), [#140](https://github.com/JonathanGodley/fastmail-mcp/issues/140)) both allow a start and an end in two different named zones — the ordinary reason is a flight, departing in one zone and landing in another; on write such a pair still has to run forwards as instants. `endTimeZone` is also omitted when `end` is absent, or was computed from a stored `DURATION` rather than a real `DTEND`: a computed end shares `start`'s zone by construction, so there is nothing to disagree about.
...
#### Busy and free: an all-day event is free, a timed event busy

The caller-facing values are **`busy`** and **`free`** — what the `transparency` parameter takes on `create_calendar_event` and `update_calendar_event`, and what the `transparency` field reports on a read. (iCalendar stores this as `TRANSP:OPAQUE`/`TRANSP:TRANSPARENT`; those spellings are not accepted as parameter values.)

**On create the default follows the event’s frame, matching the Fastmail client**: an all-day event is created **free**, a timed event **busy** ([#195](https://github.com/JonathanGodley/fastmail-mcp/issues/195), measurements in [docs/fastmail-action-availability.md](docs/fastmail-action-availability.md)). Pass `transparency` to override that on either frame ([#194](https://github.com/JonathanGodley/fastmail-mcp/issues/194)) — `"busy"` for an all-day event that should block the whole day, `"free"` for a timed one you stay available during.

**`update_calendar_event` changes free/busy only when you ask it to.** `transparency` sets or replaces the value; `clearFields: ["transparency"]` removes the property, which leaves the event reading as busy without saying so. Omit both and the stored value survives everything else the call does — **including a change of frame, in either direction**. An all-day event you convert to a meeting stays marked free, and a timed event you convert to all-day keeps whatever it had, so pass `transparency` alongside the new dates when the converted event should read differently.

**The two read tools differ on purpose.** A `list_calendar_events` row carries `transparency` only when the event is not busy, so a row that says nothing is busy and ordinary rows stay small. `get_calendar_event` states it on **every** event. Both report `"busy"` or `"free"`, or — where the record holds a token the spec does not define — that token verbatim.
...
├── draft-email-handler.ts # draft_email orchestration behind an injected client
...
├── reply-quote.ts          # Builds the reply-quote, forwarded-message and signature blocks
