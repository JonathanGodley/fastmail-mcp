...
2. Install into Claude Desktop:
   ...
   | Email Date Timezone | `FASTMAIL_TIMEZONE` | Blank uses the host's zone. Also governs calendar window dates and `create_calendar_event`'s default zone — an available option, not a requirement. |
...
Where the underlying value comes from: `list_calendar_events` with an expansion window normalises most in-window *occurrences* to a UTC instant server-side, so those rows carry `Z` and no `timeZone`; an all-day row, and any row expansion did not touch, keeps whatever form the stored property had, judged by the same table. `get_calendar_event` and an unwindowed `list_calendar_events` call return the stored property exactly as written.
...
- `null`, an empty string, or a whitespace-only string — there is no way to force a genuinely floating write through this parameter. Note that a read returns `timeZone: null` for a floating event, so a read-modify-write call that echoes that straight back will hit this rejection; pass no `timeZone` at all to leave a floating value floating.
