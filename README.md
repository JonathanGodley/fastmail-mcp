...
- **edit_draft**: Edit an existing draft email. Since JMAP emails are immutable, this creates a replacement draft and then deletes the old one (so the returned email ID is new). The edit preserves the draft's threading headers (In-Reply-To/References), attachments, and other keywords. On the rare failure where the replacement is created but the old copy can't be removed, you may be left with a duplicate draft rather than none. A draft containing inline (`cid:`) images, or a body part that isn't plain text or HTML, can't be preserved by editing and is **rejected** — recreate it instead (see [#13](https://github.com/JonathanGodley/fastmail-mcp/issues/13)). Only fields you provide are changed; omit a field to leave it unchanged.
  ...
  - **Cross-format coupling.** Writing one body (`textBody` or `htmlBody`) makes the draft single-format in that body. If the draft already has the other body, the edit is **rejected** rather than silently discarding it (recipients render the HTML alternative, so a half-updated pair would ship stale content). Resolve it one of two ways: supply **both** bodies to keep a synced pair, or list the other body in `clearFields` to drop it deliberately. A subject/recipient-only edit (no body written) preserves both bodies. This guard catches accidental clobber of `create_draft`/`send_draft` divergence; it does **not** validate that two non-empty bodies say the same thing.
- **send_draft**: Send an existing draft email. The draft must have recipients and a from address. Moves the email to the Sent folder. If the draft carries an **empty body part** (e.g. a blank `htmlBody` alongside real text, which can happen for drafts created in other clients), sending is **rejected**: an empty `text/html` part renders blank and shadows a real `text/plain`, so the recipient would see nothing. Edit the draft to supply or clear that body first. (Drafts created by this server never carry an empty part; every send/draft path drops empty bodies on write.)
