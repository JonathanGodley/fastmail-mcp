...
`endTimeZone` describes `end`, but only **relative to `start`**: omitted whenever `end`'s zone matches `start`'s (the ordinary case), and otherwise set by the same rule as the table above. RFC 5545 §3.8.5.3 and this server's own write-side check ([Start/end agreement](#startend-agreement), [#140](https://github.com/JonathanGodley/fastmail-mcp/issues/140)) both allow a start and an end in two different named zones — the ordinary reason is a flight, departing in one zone and landing in another. `endTimeZone` is also omitted when `end` is absent, or was computed from a stored `DURATION` rather than a real `DTEND`: a computed end shares `start`'s zone by construction, so there is nothing to disagree about.
