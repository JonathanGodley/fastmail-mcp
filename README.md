...
- **Outgoing attachments, including embedded images** — attach local files to a draft, reply, forward, or edited draft (append / remove-by-ref / clear-all), and give an item a `cid` to show it *inside* the message body instead of hanging it off the end ([#13](https://github.com/JonathanGodley/fastmail-mcp/issues/13)). Disabled until you opt in by setting `FASTMAIL_ATTACH_DIR`, and confined to that directory (reading a local file to email it out is an exfiltration vector). See [Sending attachments](#sending-attachments).
...
`create_draft`, `reply_email`, `forward_email`, and `edit_draft` accept an `attachments` array (on `forward_email` these are NEW files to add — the original's own attachments are carried separately, governed by `includeOriginalAttachments`/`asAttachment`, and the images its body displayed are carried as body content; see [Replying and forwarding with images](#replying-and-forwarding-with-images)). Each item is an object:
...
Embedding needs the same opt-in as any attachment: without `FASTMAIL_ATTACH_DIR` there is no file to embed, and the refusal says so instead of pointing you at a parameter that can't work.
