...
- **List parameters take a bare string too** - a single value, a comma-separated string or a JSON-encoded array all coerce to an array. That covers `to`/`cc`/`bcc`/`replyTo`, `emailIds`/`mailboxIds`, `clearFields`, `removeAttachments`, `fields`, and `inReplyTo`/`references` on `draft_email`'s `mode:'new'`.
  - **Not every one of them yet *declares* the string form in its schema, and that matters for a client that validates arguments before sending.** Where the schema says array-or-string the stringified form reaches the coercion; where it still says array alone, a validating client rejects it first and the leniency above is unreachable through that client. The ones that declare it today are `fields` on every read tool, `to`/`cc`/`bcc`/`replyTo` and `inReplyTo`/`references` on `draft_email`, `participants` on the calendar write tools, `emailIds` on `archive_email`, and `clearFields`/`removeAttachments` on `edit_draft`. A client that does not validate is unaffected either way.
