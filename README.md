...
>
> **Wildcard identities are not addresses:** a Fastmail account can hold a *wildcard* identity, whose address is the pattern `*@example.com` rather than a real address. It verifies, and signs, any concrete address in that domain — but the pattern itself must never reach a From header, so **a pattern is refused wherever it would be used as an address** ([#160](https://github.com/JonathanGodley/fastmail-mcp/issues/160)). That is three places: `draft_email` with no `from` while the account default is a wildcard; `edit_draft` with no `from` on a draft that carries none either; and `send_draft` on a draft whose stored From is itself a pattern (the heal path for a draft written before this refusal existed, or by another client — note that such a draft passes the "matches a verified identity" check, since the pattern equals the wildcard identity it came from). **Everything else composes normally.** Passing a concrete `from` in that domain works, is verified by the wildcard identity and carries its signature; and a draft that already stores a concrete From edits normally, even when that address matches no identity at all — refusing there would make a draft composed under a wildcard identity permanently uneditable.
