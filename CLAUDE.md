...
This repo has CI, and it runs on a different platform from the one you are on. `.github/workflows/test.yml` runs build, the test-glob floor check, `typecheck:tests` and `npm test` on ubuntu-latest across Node 20, 22 and 24, on every push to `main` and every PR; `npx-smoke.yml` packs the tarball and boots the installed bin on the same matrix; `secret-scan.yml` runs `scan-secrets.mjs --all`. Nothing is required to pass before a push lands, so a red `main` is discovered rather than prevented — check the run after pushing (`gh run list --repo JonathanGodley/fastmail-mcp --limit 3`).
