...
Handler logic must be unit-testable. The `index.ts` CallTool `switch` has no test harness, so logic left inline there can only be exercised by a live run — which is not durable regression protection. When a handler does more than trivially destructure-and-delegate (orchestration, branching, threading uploaded data into multiple paths), extract it into a function that takes an injected client and unit-test it with a mock. The compose path is the pattern: `composeDraftEmail(args, client, attachDir)` in `src/draft-email-handler.ts` takes a `DraftEmailClient` interface (which `JmapClient` satisfies structurally), so the mode branches, the token expansion and the attachment-threading are all covered in `npm test` with no credentials and no network. The handler is then a thin result→text wrapper.
...
A new pin is proved by a lever that turns it red at that assertion. A lever that trips an earlier assertion proves nothing about a later one — ask what a mutation could reach without failing the test. And a claim about a set (every call site, every tool, every field) is proved by enumerating the set, never by sampling it.

`scripts/mutate-commit.mjs <commit> [--tests=a,b]` runs Stryker over only the lines that commit changed, against only the matching test files; a survived mutant is an untested line. Use it in place of hand-reverting a change to prove a test fails without it. It refuses to run unless `<commit>` is the repo's own current HEAD and its tree is clean, since Stryker copies whatever it finds on disk — a non-tip commit of a multi-commit branch needs a detached checkout (or a worktree) first. Two limits are inherent to the scoping, not bugs: a line the commit did not touch is never mutated (a boundary condition just above a changed block is invisible to the run), and the test files are chosen by name — `src/X.ts` → `src/X*.test.ts` plus whatever tests the commit itself changed — so coverage that lives in a differently-named file reads as a survivor. Pass `--tests` to set the list by hand; that is also how you leave out two unrelated kinds of test that do not behave well here, and they fail in different ways. The first is any of five tests that locate something in `src/index.ts` by matching its exact text rather than parsing it: Stryker reprints a mutated file WHOLE, not just around the mutant, so a change anywhere in the file can break a match nowhere near it. `readme-inventory.test.ts` and `tool-schema.test.ts` locate the `const TOOLS = [` array and a recipient-parameter description this way; `config-surface.test.ts` and `jmap-client.test.ts`'s "version sync" check each match one narrow literal and fail only when that specific text is disturbed. `index-env.test.ts` fails for a different reason again, and unconditionally: Stryker's own instrumentation header, added to whichever file gets a mutant, itself reads `process.env`, which this guard's file-wide scan flags - it fails on every run that selects it, whichever `src/*.ts` file is mutated. `index-env.test.ts` and `jmap-client.test.ts` are the only two of the five the by-name rule above ever auto-selects on its own (for `src/index.ts` and `src/jmap-client.ts` respectively). Only `jmap-client.test.ts` is also that module's real unit-test suite; the other four, `index-env.test.ts` included, exist for no reason but the guard - for a commit touching both files, check whether `jmap-client.test.ts`'s version-sync check actually fails before excluding the whole file along with `index-env.test.ts`. The second kind is `dist/index.js`: it is gitignored, but that does not keep it out of a sandbox, since Stryker copies whatever the tree has on disk regardless of `.gitignore`. `built-server.test.ts`'s `before` hooks fail the initial run outright either way - "does not exist" with no build, a stale-mtime assertion with a build present, because Stryker's sandbox copy does not preserve file timestamps. `server-lifecycle.test.ts` only skips its suite when `dist/` is absent; built, it runs and spawns the last build, so it can never register a kill either way. For a change to `src/index.ts` itself, no `--tests` selection avoids the underlying gap: every test pinning its content either is one of the five text-parsing guards above, or is one of these two `dist/`-reading tests, so it either fails outright or measures the last build, not the mutated source.

It runs from a `git worktree` as well as from the primary checkout, so the implementer who wrote a line proves it tested in their own tree rather than deferring the check to a pass on `main` after the branch lands. The worktree supplies the files to mutate; the primary checkout supplies the installed `node_modules`, because `npm install` is run once and a worktree has none of its own (#178). That is the one thing crossing between the two trees, and it means an initial-run failure naming a missing package is a dependency-resolution problem, not a test-selection one: `--tests` is no help against it, however much the message resembles the drift-guard failure above.

A guard that reads source as text is proved red by hand, with three levers. Stryker cannot reach it (above), so the mutation is yours to make and revert: delete the pinned sentence from one of the sites it is asserted at; re-word the shared constant so the pinned phrase no longer appears while the site still carries a description; delete a whole property block so the site is gone rather than wrong. Run each lever until the guard fails at the assertion aimed at that case — a lever that trips an earlier assertion, or the vacuity floor, proves nothing about the pin it was meant for. Say in the commit message which levers went red and where; the Stryker survivors on such a file are then a recorded limit rather than an open question.

This repo has CI, and it runs on a different platform from the one you are on. `.github/workflows/test.yml` runs build, the test-glob floor check, `typecheck:tests` and `npm test` on ubuntu-latest across Node 20, 22 and 24, on every push to `main` and every PR; `npx-smoke.yml` packs the tarball and boots the installed bin on the same matrix; `secret-scan.yml` runs `scan-secrets.mjs --all`. Nothing is required to pass before a push lands, so a red `main` is discovered rather than prevented — check the run after pushing (`gh run list --repo JonathanGodley/fastmail-mcp --limit 3`; `--commit $(git rev-parse HEAD)` narrows it to the push, and a short SHA there matches nothing).

Local green is therefore a weaker claim than it looks, and the gap is not hypothetical: `302c326` fixed a path-confinement fixture that hard-coded three 55-character segments and assumed a long `tmpdir()`. That is true of `C:\Users\JG\AppData\Local\Temp` and false of `/tmp`, so the assertion `fixture path must exceed the echo bound` passed on Windows and failed on Linux, and `main` went red. Any fixture whose arithmetic depends on path length, path separator or line endings is invisible to a Windows-only run by construction — derive the value from the actual temp root at run time rather than assuming its length, and treat CI, not the local suite, as the finish line for a change that touches one.
...
A comment earns its place by changing what the next reader would do. `attachMailboxInfo` is the model: its comment says why the resolver neither throws nor omits, and that is the bar for every comment, a declined finding's included - a competent reader about to change the line would otherwise get it wrong. State a fact once, at the definition, and reference it from call sites; a decision nobody challenged needs no defence beside it. How the code got here (which review found what, what an earlier patch proposed) belongs in the commit message and the issue; anything spanning several tools belongs in `docs/`. `node scripts/comment-share.mjs <commit>` measures a change against the file's own comment density; a figure far above it is nearly always restatement or history, not domain complexity - `/tidy-comments` is the pass that cuts it.

**Fix it rather than filing it.** A small defect with an obvious fix is fixed in the change in front of you — or, where nothing is in flight, in the pass that found it — never parked as an issue: a build that touches an area owns the defects it finds there. A defect that would leave the merged build broken is never trackable: if it genuinely cannot be fixed now, the build does not merge and it goes to the operator. File a finding only when the work genuinely cannot land now — the fix is design work of its own (judged on the minimal fix, not the ideal one), it needs a decision with a real trade-off nobody has made, it is the residual half of a partly-fixed change, or the area is fenced off from the current session. A review that surfaces a question does not file the question: it answers it, or surfaces it to the operator. ("Track" is the cheapest disposition to justify and reads as diligence while being avoidance — it is how this tracker came to hold nearly as much unfinished work as finished.) Upstream bookkeeping is exempt: the per-PR adoption issues and the `docs/upstream-sync.md` ledger are not findings and are filed unfiltered.
...
- Cross-cutting rationale → the `docs/*.md` files (checked in, version-controlled). Facts and models that span multiple tools, or that are properties of the JMAP/Fastmail platform or the shared codebase:
  ...
  - `docs/conventions.md` — the sending-identity model (RFC 8621 §6.1's split between an enforced `email` and an unvalidated `name`, Cyrus's stub Identity implementation, and the three consequences this server draws from it), lenient input coercion (and its fail-closed variant for arguments that narrow what a call touches), mailbox-query scoping (JMAP's singular `inMailbox`, the solely-in `inMailboxOtherThan`, the single unioned excluded set, and the two sites that decide whether the default Trash/Spam exclusion runs), the U+202F local-time trap, result serialisation (the two compact seams, and what the drift guard over them does and does not buy - it is not redaction), untrusted values in prose (the one rule every thrown or returned message follows - redact then neutralise, sanitise the VALUE and never the finished sentence - and the module boundary that is still outside it), calendar window bounds (a date-only window is the caller's LOCAL day, and a one-sided window is bounded and says so), free/busy across the calendar tools (one caller vocabulary, the read split, and create decides a default where update never does), the two-pass quote sanitiser (its posture, and why a quote is rebuilt in two passes), and dependency/build gotchas.
