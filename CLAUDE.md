...
**Fix it rather than filing it.** A small defect with an obvious fix is fixed in the change in front of you — or, where nothing is in flight, in the pass that found it — never parked as an issue: a build that touches an area owns the defects it finds there, and a defect that would leave the merged build broken is never trackable, so if it genuinely cannot be fixed now the build does not merge and it goes to the operator. "Track" is the cheapest disposition to justify, and its artifact is the one that looks like progress without being any, so it reads as diligence while being avoidance; it is how this tracker came to hold nearly as much unfinished work as finished. The test for filing a finding is that the work genuinely cannot land now — for example the fix is design work of its own (judged on the minimal fix, not the ideal one), it needs a decision with a real trade-off nobody has made, it is the residual half of a partly-fixed change, or the area is fenced off from the current session — and a review that surfaces a question does not file the question: it answers it, or surfaces it to the operator. None of this touches the upstream bookkeeping: the per-PR adoption issues in **Working with upstream** and the not-adopted ledger in `docs/upstream-sync.md` are not findings, and are filed unfiltered as those sections say.
