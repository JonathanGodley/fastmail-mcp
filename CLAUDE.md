...
`scripts/mutate-commit.mjs <commit> [--tests=a,b]` runs Stryker over only the lines that commit changed, against only the matching test files; a survived mutant is an untested line. Use it in place of hand-reverting a change to prove a test fails without it. It refuses to run unless `<commit>` is the repo's own current HEAD and its tree is clean, since Stryker copies whatever it finds on disk — a non-tip commit of a multi-commit branch needs a detached checkout (or a worktree) first. Two limits are inherent to the scoping, not bugs: a line the commit did not touch is never mutated (a boundary condition just above a changed block is invisible to the run), and the test files are chosen by name — `src/X.ts` → `src/X*.test.ts` plus whatever tests the commit itself changed — so coverage that lives in a differently-named file reads as a survivor. Pass `--tests` to set the list by hand; that is also how you leave out two unrelated kinds of test Stryker cannot run. The first parses a `src/*.ts` file's own text and breaks the moment Stryker's instrumentation reaches the line it matches: `index-env.test.ts`, `readme-inventory.test.ts`, `tool-schema.test.ts`, `config-surface.test.ts`, and `jmap-client.test.ts`'s "version sync" check all locate a literal or a declaration in `src/index.ts` by matching its exact line. Of these five, only `index-env.test.ts` shares a source-file stem with `src/index.ts` and is ever picked up by the by-name rule above; the other four only enter a run through an explicit `--tests` or by being changed in the same commit. The second reads `dist/index.js`, which is gitignored and never reaches a sandbox: `built-server.test.ts`'s `before` hooks assert it is present and current and fail the initial run outright every time; `server-lifecycle.test.ts` instead skips its whole suite when `dist/` is missing, so it does not fail a run, but selecting it never contributes a kill either. For a change to `src/index.ts` itself, no `--tests` selection avoids the underlying gap: every test pinning its content either is one of the five structural guards above, or is one of the two `dist/`-reading tests above and so either fails outright or measures the last build, not the mutated source.
