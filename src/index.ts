      ...
      },
      {
        ...
        inputSchema: {
          ...
          properties: {
            ...
            from: {
              type: 'string',
              description: 'Sender email address (optional, defaults to account primary email). It also picks the identity whose signature {{signature}} expands to. If your account\'s default identity is a WILDCARD — its address is the pattern "*@example.com" rather than an address — omitting from is refused, because the pattern would be written into the From header as the sender. Name a concrete address in that domain here; the wildcard identity still verifies it and still supplies its signature.',
            },
            replyTo: {
              type: ['array', 'string'],
              items: { type: 'string' },
