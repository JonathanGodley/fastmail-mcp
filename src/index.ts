  ...
  try {
    ...
    switch (name) {
      ...
      case 'get_calendar_event': {
        ...
        const { event, brokenCollections } = await davClient.getCalendarEventById(eventId);
        return {
          content: [{ type: 'text', text: `${toolJson(event)}${buildBrokenCollectionNote(brokenCollections, 'read')}` }],
        };
      }
