...
const TOOLS = [
      ...
      },
      {
        ...
        description:
          ...
          'A SINGLE REPEATING EVENT TOO DENSE TO MATERIALISE IS LEFT OUT, whatever window you named: a series that expands to more than 5000 blocks in the RANGE THIS SERVER SEARCHED for your window is omitted from the rows AND from the total, and named in a trailing "Note:" line with its title, id, occurrence count and calendar. That range runs 14 hours past each edge of the window you named (see the widening described above), so the count reported is over the returned blocks and can exceed the number of occurrences falling strictly inside your window. The span is yours to choose, but the density is not — one FREQ=MINUTELY invitation would otherwise fill the whole page and push every real event off it. The call still answers everything else normally; narrow the window to see the omitted series. ' +
