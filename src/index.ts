...
const TOOLS = [
      ...
      },
      {
        ...
        description:
          ...
          'A SINGLE REPEATING EVENT TOO DENSE TO MATERIALISE IS LEFT OUT, whatever window you named: a series expanding to more than 5000 occurrences inside the window is omitted from the rows AND from the total, and named in a trailing "Note:" line with its title, id, occurrence count and calendar. The span is yours to choose, but the density is not — one FREQ=MINUTELY invitation would otherwise fill the whole page and push every real event off it. The call still answers everything else normally; narrow the window to see the omitted series. ' +
