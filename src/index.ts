...
// the flag means (edit_draft has no appendSignature at all — it expands a `{{signature}}`
// you placed, behind expandSignature, and appends nothing on its own). `context` names where
// the signature lands relative to whatever else that tool puts in the body.
//
...
const TOOLS = [
      ...
      {
        name: 'get_email',
        description: 'Get a specific email by ID. Returns simplified format with plain text body (HTML omitted, bodyHtmlSize hint provided). Only use verbose=true if you specifically need the HTML body — it can be very large for marketing emails. Use raw=true for original JMAP response. Set stripQuoted=true to drop quoted reply history from bodyText when reading a message deep in a long thread (the quoted tail is duplicated from earlier messages). The date field is rendered in local time with a UTC offset (e.g. 2026-03-02T08:00:00+10:00), not UTC; raw=true returns the canonical JMAP UTC time. ' + LOCATION_FIELDS_DESC + ' ' + UNION_SCOPE_DESC + ' ' + INLINE_PAIR_DESC + ' ' + MINTED_CID_NONDURABILITY + ' READING A DRAFT ALSO RETURNS bodyHash, the token edit_draft requires before it will write or clear that draft\'s body. This is the only tool that issues one. When it cannot be issued the read returns bodyHashWithheld saying why and naming the read that would issue one — never silence — which happens when the response does not show the whole stored body: a truncated part, a projection that dropped a body field, or stripQuoted:true (which returns a shortened body). raw:true returns neither field, and get_thread never issues one, so read the draft here before editing it. ' + FIELDS_TOOL_DESC + ' On this tool fields:["bodyHtml"] returns the HTML body ALONE (no verbose needed, no metadata, no plain-text copy) — the way to read a large HTML draft without the rest of the message pushing the response past the output limit; a body field in the projection brings its bodyHash/bodyHashWithheld along, so that read is enough to edit from.',
        inputSchema: {
          type: 'object',
          properties: {
            emailId: {
      ...
      },
      {
        ...
        inputSchema: {
          ...
          properties: {
            ...
            bodyHash: {
              type: 'string',
              description: "Proof that you are replacing the body you read: the `bodyHash` value get_email returned for this draft. REQUIRED on every edit that writes textBody/htmlBody or clears one of them; ignored on a metadata-only edit, which leaves both bodies untouched. An absent or stale hash is rejected — re-read the draft with get_email and edit from that. It is a lost-update guard and nothing more: it proves you SAW the body, never that you kept any of it, so the check passes on an edit that replaces the body entirely. A successful body edit hands back the hash for your next edit of the same draft, so a run of edits needs one get_email at the start.",
            },
            replyTo: {
              type: 'array',
              items: { type: 'string' },
            ...
            clearFields: {
              ...
              description: "Field names to deliberately clear (to empty/none). Allowed: to, cc, bcc, replyTo, subject, textBody, htmlBody, attachments, forwardedMessageId. `from` cannot be cleared. Cannot also pass the same field as a value (e.g. attachments + clearFields:['attachments'] is rejected). Clearing textBody or htmlBody requires bodyHash. clearFields:['attachments'] takes off every part, images embedded in the body included, and is rejected when the surviving body still references one of them (rewrite or clear that body in the same call). clearFields:['forwardedMessageId'] de-forwards the draft: it drops the recorded X-Forwarded-Message-Id so send_draft will not mark the original forwarded. That is metadata, so it works on a body edit and a metadata-only edit alike, and it does NOT touch the body — a forwarded-message block already in the body stays there until you replace the body yourself.",
            },
          },
          required: ['emailId'],
        },
