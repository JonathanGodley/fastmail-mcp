...
const TOOLS = [
      ...
      },
      {
        ...
        description:
          ...
          `CALENDAR TIMES CARRY A ZONE NAME, NEVER AN OFFSET. \`start\`/\`end\` is a bare local wall clock (2026-04-20T10:00:00), a Z-designated UTC instant, or a date-only (all-day) value. \`timeZone\` names the IANA zone a wall-clock \`start\` is in, but ONLY when it differs from this server's configured zone (${CONFIGURED_TIMEZONE}): an ABSENT \`timeZone\` means ${CONFIGURED_TIMEZONE}, and \`timeZone: null\` means \`start\` is genuinely FLOATING (RFC 5545 §3.3.5 — a different instant for every reader), which is a different fact from "in the configured zone". A Z-designated value or an all-day value never carries \`timeZone\`, because both already name themselves. \`endTimeZone\` describes \`end\` relative to \`start\` — it appears only when \`end\`'s zone differs from \`start\`'s, which is legal (a flight departing one zone and landing in another), and is otherwise omitted. This path returns the stored property exactly as written, with no normalisation, so it is where a floating or differently-zoned value is most likely to show up. Same rule as list_calendar_events.`,
        inputSchema: {
          type: 'object',
          properties: {
            eventId: {
