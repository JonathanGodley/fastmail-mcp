...
const CALENDAR_EVENT_ID_DESC =
  ...
  'AN ID CAN NAME MORE THAN ONE RECORD: a UID is unique within one calendar, not across the account, so two calendars can ' +
  'hold the same id. Where they do, update_calendar_event and delete_calendar_event REFUSE it outright and name every copy ' +
  'with its calendar and its `url`, rather than picking one; get_calendar_event answers with the FIRST copy, adds an ' +
  '`otherCopies` list naming the rest, and says so in a trailing "Note:" line. Pass a copy\'s `url` as eventId to act on ' +
  'that copy — a url that still has a record at it ADDRESSES exactly one, so it is not the ambiguous case. That holds ' +
  'even when some OTHER record carries that same url as its UID: the copy at the address is the one get_calendar_event ' +
  'returns first and the one the write tools act on, and the rest are still listed. THE ONE EXCEPTION: a url whose ' +
  'record has since been DELETED addresses nothing, so it falls back to being matched as a UID like any other id — and ' +
  'if two records carry it as their UID, that is ambiguous and the write tools refuse it. THE ACCEPTED CONSEQUENCE: whoever sends this ' +
  'account an invitation chooses the UID it arrives under, so a stranger can freeze an event\'s writes by minting a ' +
  'duplicate of an id they already know. The url form is the guaranteed way through, on every one of these tools.';
...
const TOOLS = [
        ...
        inputSchema: {
          ...
          properties: {
            mode: {
              ...
              description: 'Which kind of draft to compose. REQUIRED, with no default — a defaulted mode would turn a forgotten parameter into a silently unthreaded new message. "new" composes a fresh message and accepts no history token. "reply" threads against originalEmailId (In-Reply-To/References, "Re: " subject, recipients defaulting to reply-all) and accepts {{quote}}. "forward" starts a new conversation carrying originalEmailId, records it in X-Forwarded-Message-Id, carries the original\'s attachments, requires `to`, and accepts {{forward}} (or asAttachment instead). A value that is not one of the three is refused naming all three; it is not guessed at.',
            },
            originalEmailId: {
              type: 'string',
            ...
            from: {
              type: 'string',
              description: 'Sender email address (optional, defaults to account primary email). Takes a bare address or the "Name <email>" form the recipient fields take: the ADDRESS half must match one of your verified identities, and the display name half is never validated, because nothing on the platform reads it — it is simply written into the From header. Omit the name and the matched identity\'s configured name is written instead, so a bare from behaves exactly as it always has. It also picks the identity whose signature {{signature}} expands to, again by the address half. If your account\'s default identity is a WILDCARD — its address is the pattern "*@example.com" rather than an address — omitting from is refused, because the pattern would be written into the From header as the sender. Name a concrete address in that domain here; the wildcard identity still verifies it and still supplies its signature. PASSING THE PATTERN ITSELF IS REFUSED TOO, bare or as the ADDRESS half of the "Name <email>" form: it is a pattern in either spelling, and it verifies against the wildcard identity it came from, so nothing further down would catch it. (The NAME half is never validated, here as anywhere, so a display name that happens to contain an asterisk is fine.) A wildcard is also where naming your own display name matters most: one configured name stands behind every address in the domain, so without a name of your own that single name is the only sender name any of those addresses can carry.',
            },
            replyTo: {
              type: ['array', 'string'],
              items: { type: 'string' },
      ...
      },
      {
        ...
        inputSchema: {
          ...
          properties: {
            ...
            from: {
              type: 'string',
              description: 'Updated sender email address (optional). Takes a bare address or the "Name <email>" form the recipient fields take: the ADDRESS half must match one of your verified identities, and the display name half is never validated. A name you write here WINS — it is a passed field like any other, and it overwrites the name the draft already stored. Pass a bare address instead and the draft\'s own stored name is kept, so an edit that is not about the sender name never changes one. Switching, with no name of your own, to a different address you can send as writes that identity\'s display name, since the draft carries no name for the new address yet. NOTE that a display name cannot be REMOVED here, only replaced: from is not clearable, and a from carrying no name (a bare address, or "<addr>") means "no name of my own", so it defers to the stored one. Omitting from keeps the address the draft already carries, whatever identity that address matches — including a draft composed under a WILDCARD identity, which edits normally. The one refusal on an omitted from is a draft that carries no From at all while the account\'s default identity is a wildcard (its address is the pattern "*@example.com"): there is then no concrete address to write, so pass one here. PASSING THAT PATTERN IS REFUSED TOO, bare or as the ADDRESS half of the "Name <email>" form: it is a pattern in either spelling, and it verifies against the wildcard identity it came from, so nothing further down would catch it — pass a concrete address in its domain. (The NAME half is never validated, so a display name that happens to contain an asterisk is fine.)',
            },
            subject: {
              type: 'string',
      ...
      },
      {
        ...
        description:
          'List all calendars. ' +
          'A COLLECTION THE SERVER FAILED TO DESCRIBE IS NAMED RATHER THAN DROPPED: when an entry inside the account\'s calendar home comes back broken, it cannot appear as a row — the failure destroys its display name and its type together — so the response carries a trailing "Note:" line CONTAINING the phrase "' + BROKEN_COLLECTION_PHRASE + '", followed by its path and then by what that failure does and does not mean. The line OPENS "Note: a collection ' + BROKEN_COLLECTION_PHRASE + ': " for one and "Note: 3 collections ' + BROKEN_COLLECTION_PHRASE + ': " for several, and runs on well past there — so match on the phrase, never on a whole line and never on the singular subject. It is not known whether that collection was a calendar, and the note does not claim it was; treat the listing as the calendars that DID answer. The same note appears on list_calendar_events and get_calendar_event, and the write tools name the collection they could not look in. A calendar list built while one was broken is NEVER cached, so the next calendar call re-asks the server. ' +
          'THE RESIDUAL, so it is not mistaken for a guarantee: a healthy list IS cached for the life of the process, so a calendar that breaks AFTER a successful discovery keeps being listed here until the server restarts. This detection runs when the list is built, not on every call.',
        inputSchema: {
          type: 'object',
          properties: {},
        },
      ...
      },
      {
        ...
        description:
          ...
          ' AN AMBIGUOUS ID IS REFUSED: where two calendars hold the same id this destroys neither, and names every copy with its calendar and its `url` so you can pass the one you mean — see the `eventId` parameter.' +
          ...
          ' A COLLECTION THAT FAILED TO LIST DOES NOT BLOCK THE DELETE: when an entry in the account\'s calendar home came back broken, this call still destroys the copy it found in a collection that did answer, and the response carries a trailing "Note:" line whose subject ends "' + BROKEN_COLLECTION_PHRASE + '" (the subject counts them: "a collection …", or "N collections …"; the line itself continues past the path) naming what it could not look in — so where the SAME UID also exists in the collection that failed, that copy survives. A "Calendar event not found" error carries the same path.',
        inputSchema: {
          type: 'object',
          properties: {
            eventId: {
  ...
  try {
    ...
    switch (name) {
      ...
      case 'create_calendar_event': {
        ...
        return { content: [{ type: 'text', text: `Calendar event created. Event ID: ${result.eventId}.${describeCreateCalendarEventResult(result)}${buildBrokenCollectionNote(result.brokenCollections, 'create')}` }] };
      }
      ...
      case 'update_calendar_event': {
        ...
        if (title === undefined && description === undefined && start === undefined && end === undefined && location === undefined && participants === undefined && timeZone === undefined && transparency === undefined && !hasClearFields) {
          throw new McpError(ErrorCode.InvalidParams, 'At least one field to update must be provided (title, description, start, end, location, participants, timeZone, transparency, or clearFields)');
        }
        const davClient = initializeCalDAVClient();
        if (!davClient) {
          throw new McpError(ErrorCode.InvalidRequest, 'CalDAV not configured. Set FASTMAIL_CALDAV_USERNAME and FASTMAIL_CALDAV_PASSWORD.');
      ...
      case 'delete_calendar_event': {
        ...
        return { content: [{ type: 'text', text: `Calendar event deleted. Event ID: ${eventId}${buildBrokenCollectionNote(deleted.brokenCollections, 'write')}` }] };
      }
