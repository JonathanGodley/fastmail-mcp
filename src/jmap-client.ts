...
export class JmapClient {
  ...
  /**
   * Extract a QueryResult (items + total) from a JMAP query+get response pair.
   * @param listIndex  index of the /get response (for the items list)
   * @param queryIndex index of the /query response (for the total count)
   */
  protected getQueryResult(response: JmapResponse, listIndex: number, queryIndex: number): QueryResult {
    const items = this.getListResult(response, listIndex);
    const queryResult = this.getMethodResult(response, queryIndex);
    const total = queryResult?.total ?? items.length;
    return { items, total };
  }
    ...
    const request: JmapRequest = {
      ...
      methodCalls: [
        ...
        ['Email/get', {
          ...
          properties: ['id', 'subject', 'from', 'to', 'cc', 'bcc', 'replyTo', 'receivedAt', 'preview', 'keywords', 'threadId', 'messageId', 'references', 'inReplyTo', 'hasAttachment']
        }, 'emails']
      ]
    };
    ...
    const request: JmapRequest = {
      ...
      methodCalls: [
        ['Email/get', {
          accountId: session.accountId,
          ids: [id],
          properties: ['id', 'subject', 'from', 'to', 'cc', 'bcc', 'replyTo', 'receivedAt', 'preview', 'hasAttachment', 'textBody', 'htmlBody', 'attachments', 'bodyValues', 'messageId', 'threadId', 'inReplyTo', 'references', 'keywords'],
  ...
  async getThread(threadId: string, compact: boolean = true): Promise<any[]> {
    ...
    const emailGetParams: Record<string, any> = {
      ...
      properties: compact
        ...
        : ['id', 'subject', 'from', 'to', 'cc', 'bcc', 'replyTo', 'receivedAt', 'preview', 'hasAttachment', 'keywords', 'messageId', 'references', 'threadId', 'textBody', 'htmlBody', 'attachments', 'bodyValues', 'inReplyTo'],
    };
    if (!compact) {
