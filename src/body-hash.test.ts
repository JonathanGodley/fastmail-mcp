...
describe('resolveDraftBodyHash', () => {
  ...
  // A typeless part is an ordinary body to the READ: the reader displays it in whichever
  // list carries it, so the hash covers it and is issued. The interleaved check does not
  // count it, so it pairs with nothing and earns no refusal.
